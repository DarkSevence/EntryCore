...
class TargetBoard(ui.ThinBoard):
	...
	BUTTON_NAME_LIST = ( 
		...
		localeInfo.TARGET_BUTTON_DISCONNECT,
		"Zapisz Nick",
		"VOTE_BLOCK_CHAT",
	)
	...
	def __init__(self):
		...
		self.buttonDict[localeInfo.TARGET_BUTTON_DISCONNECT].SAFE_SetEvent(self.__OnDisconnect)
		self.buttonDict["Zapisz Nick"].SAFE_SetEvent(self.__OnSaveNick)
	...
	def ShowDefaultButton(self):
		...
		if str(player.GetName())[0] == "[":
			...
			self.__ShowButton(localeInfo.TARGET_BUTTON_DISCONNECT)
			self.__ShowButton("Zapisz Nick")
	...
	def __OnDisconnect(self):
		net.SendChatPacket("/dc %s" % self.nameString)

	def __OnSaveNick(self):
		import os
		
		if os.path.exists("config/ignore.cfg"):
			SaveNickTest = open("config/ignore.cfg", "r")
			SaveNickList = SaveNickTest.read()
			SaveNickPlayer = SaveNickList.split(",\n")
			SaveNickTest.close()
			for i in xrange(str(SaveNickList).count(",\n")):
				if str(SaveNickPlayer[i]) != self.nameString:
					pass
				else:
					return
					
			SaveNick = open("config/save_nick.cfg", "a")
			SaveNick.write("%s,\n" % self.nameString)
			SaveNick.close()
		else:
			SaveNick = open("config/ignore.cfg", "w+")
			SaveNick.write("%s,\n" % self.nameString)
			SaveNick.close()
