...
class CharacterWindow(ui.ScriptWindow):
	...
	def __del__(self):
		super(CharacterWindow, self).__del__()

		attributes = [
			'toolTip', 'toolTipJob', 'toolTipAlignment', 'toolTipSkill',
			'faceImage', 'statusPlusLabel', 'statusPlusValue', 'activeSlot',
			'tabDict', 'tabButtonDict', 'pageDict', 'titleBarDict',
			'statusPlusButtonDict', 'statusMinusButtonDict', 'skillPageDict',
			'questScrollBar', 'questSlot', 'questNameList', 'questLastTimeList',
			'questLastCountList', 'skillGroupButton', 'activeSlot',
			'activeSkillPointValue', 'supportSkillPointValue', 'skillGroupButton1',
			'skillGroupButton2', 'activeSkillGroupName', 'guildNameSlot',
			'guildNameValue', 'characterNameSlot', 'characterNameValue',
			'emotionToolTip', 'soloEmotionSlot', 'dualEmotionSlot'
		]
	...
	def RefreshSkillPage(self, name, slotCount):
		global SHOW_LIMIT_SUPPORT_SKILL_LIST

		skillPage = self.skillPageDict[name]
		startSlotIndex = self.CalculateStartIndex(name, slotCount)

		for i in xrange(slotCount + 1):
			slotIndex = i + startSlotIndex
			skillIndex = player.GetSkillIndex(slotIndex)

			self.ClearSkillSlots(skillPage, slotIndex, i)

			if skillIndex == 0:
				continue

			self.HandleSkillIndex(skillPage, slotIndex, i, skillIndex)

		skillPage.RefreshSlot()
		self.RestoreSlotCoolTime(skillPage)

	def CalculateStartIndex(self, name, slotCount):
		startSlotIndex = self.skillPageDict[name].GetStartIndex()
	...
	def ClearSkillSlots(self, skillPage, slotIndex, i):
		for j in xrange(skill.SKILL_GRADE_COUNT):
			skillPage.ClearSlot(self.GetRealSkillSlot(j, i))

	def HandleSkillIndex(self, skillPage, slotIndex, i, skillIndex):
		skillGrade = player.GetSkillGrade(slotIndex)
		skillLevel = player.GetSkillLevel(slotIndex)
		skillType = skill.GetSkillType(skillIndex)

		if player.SKILL_INDEX_RIDING == skillIndex:
			self.HandleRidingSkill(skillPage, slotIndex, skillIndex, skillGrade, skillLevel)
		elif skill.SKILL_TYPE_ACTIVE == skillType:
			self.HandleActiveSkill(skillPage, slotIndex, skillIndex, skillGrade, skillLevel, i)
		else:
			self.HandleOtherSkills(skillPage, slotIndex, skillIndex, skillGrade, skillLevel)

	def HandleRidingSkill(self, skillPage, slotIndex, skillIndex, skillGrade, skillLevel):
		if skillGrade == 1:
			skillLevel += 19
		elif skillGrade == 2:
			skillLevel += 29
	...
	def HandleActiveSkill(self, skillPage, slotIndex, skillIndex, skillGrade, skillLevel, i):
		for j in xrange(skill.SKILL_GRADE_COUNT):
			realSlotIndex = self.GetRealSkillSlot(j, slotIndex)

			skillPage.SetSkillSlotNew(realSlotIndex, skillIndex, j, skillLevel)
			skillPage.SetCoverButton(realSlotIndex)

			self.UpdateSlotStatus(skillPage, realSlotIndex, skillGrade, j, skillLevel, slotIndex, i)

	def UpdateSlotStatus(self, skillPage, realSlotIndex, skillGrade, j, skillLevel, slotIndex, i):
		if (skillGrade == skill.SKILL_GRADE_COUNT) and j == (skill.SKILL_GRADE_COUNT - 1):
			skillPage.SetSlotCountNew(realSlotIndex, skillGrade, skillLevel)
		elif (not self.CanUseSkillNow()) or (skillGrade != j):
		...
		if player.IsSkillCoolTime(slotIndex) and skillGrade != j:
			skillPage.TransferSlotCoolTime(realSlotIndex, self.GetRealSkillSlot(skillGrade, i))

	def HandleOtherSkills(self, skillPage, slotIndex, skillIndex, skillGrade, skillLevel):
		if not SHOW_LIMIT_SUPPORT_SKILL_LIST or skillIndex in SHOW_LIMIT_SUPPORT_SKILL_LIST:
			realSlotIndex = self.GetETCSkillRealSlotIndex(slotIndex)
	...
	def RestoreSlotCoolTime(self, skillPage):
		restoreType = skill.SKILL_TYPE_HORSE if self.PAGE_HORSE == self.curSelectedSkillGroup else skill.SKILL_TYPE_ACTIVE
	...
	def RefreshSkill(self):
		...
		if SHOW_ONLY_ACTIVE_SKILL:
			self.RefreshSkillPage("ACTIVE", self.ACTIVE_PAGE_SLOT_COUNT)
		else:
			self.RefreshSkillPage("ACTIVE", self.ACTIVE_PAGE_SLOT_COUNT)
			self.RefreshSkillPage("SUPPORT", self.SUPPORT_PAGE_SLOT_COUNT)
			...
			for i in xrange(self.PAGE_SLOT_COUNT):
				...
				if name == "HORSE":
					if player.GetStatus(player.LEVEL) >= skill.GetSkillLevelLimit(skillIndex):
						if skillLevel < 20:
							slotWindow.ShowSlotButton(self.GetETCSkillRealSlotIndex(slotIndex))
				else:
					if "SUPPORT" == name:						
						if not SHOW_LIMIT_SUPPORT_SKILL_LIST or skillIndex in SHOW_LIMIT_SUPPORT_SKILL_LIST:
	...
	def OnUseSkill(self, slotIndex, coolTime):
		...
		if skill.SKILL_TYPE_ACTIVE == skillType:
			skillGrade = player.GetSkillGrade(slotIndex)
			slotIndex = self.GetRealSkillSlot(skillGrade, slotIndex)
		else:
			slotIndex = self.GetETCSkillRealSlotIndex(slotIndex)

		for slotWindow in self.skillPageDict.values():
			if slotWindow.HasSlot(slotIndex):
				slotWindow.StoreSlotCoolTime(skillType, slotIndex, coolTime)
				self.RestoreSlotCoolTime(slotWindow)
				return
	...
	def GetRealSkillSlot(self, skillGrade, skillSlot):
		return skillSlot + min(skill.SKILL_GRADE_COUNT - 1, skillGrade) * skill.SKILL_GRADE_STEP_COUNT

	def GetETCSkillRealSlotIndex(self, skillSlot):
		if skillSlot > 100:
			return skillSlot
