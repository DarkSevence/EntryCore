...
class GameWindow(ui.ScriptWindow):
	...
	def RefreshCommentsPage(self):
		self.interface.RefreshCommentsPage()
	...
	def OnRecvWhisperError(self, mode, name, line):
		if localeInfo.WHISPER_ERROR.has_key(mode):
			...
			error_message = "Whisper Unknown Error(mode=%d, name=%s)" % (mode, name)
			chat.AppendWhisper(chat.WHISPER_TYPE_SYSTEM, name, error_message)
	...
	def RecvPartyInviteQuestion(self, leaderVID, leaderName):
		...
		partyInviteQuestionDialog.SetText1(localeInfo.PARTY_DO_YOU_JOIN + leaderName + "?")
	...
	def SaveScreen(self):
		chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.SCREENSHOT_SAVE)
