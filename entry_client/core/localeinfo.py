...
if not os.path.exists("config/font.cfg"):
	f = open("config/font.cfg", "w")
...
APP_TITLE = "Entry-Core | Wersja: v1.9r"
...
def LoadLocaleFile(srcFileName, localeDict):
	...
	for line in lines:
		try:
			tokens = line[:-1].split("\t")
			if len(tokens) == 2:
				localeDict[tokens[0]] = tokens[1].replace('\\n', '[ENTER]').replace('/n', '[ENTER]')
			elif len(tokens) >= 3:
				type = tokens[2].strip()
				if type:
...
def SecondToDHM(time):
	...
	if day > 0:
		text += "{} {}".format(day, getPolishTimeUnit(day, ["dzieÒ", "dni", "dni"]))
		text += " "
...
MINIMAP_ZONE_NAME_DICT = {
	"map_shinsoo_capital_village"  : MAP_A1,
	"map_orc_valley"         : MAP_A2,
	"metin2_map_a3"  : MAP_A3,
	"map_chunjo_capital_village"  : MAP_B1,
	"map_b2"         : MAP_B2,
	"metin2_map_b3"  : MAP_B3,
	"map_jinno_capital_village"  : MAP_C1,
	"map_c2"         : MAP_C2,
