...
class Window(object):	
	...
	def set_click_event(self, event):
		self._click_event = weakref.ref(event)	
...
class ImageBox(Window):
	...
	def OnMouseOverIn(self):
		try:
			self.eventDict["MOUSE_OVER_IN"](self.ItemVnum)
		except KeyError:
			pass
...
class SlotWindow(Window):
	...
	def CAN_CHANGE_ICON(self):
		if item.GetItemType() == item.ITEM_TYPE_COSTUME and item.GetItemSubType() in (item.COSTUME_TYPE_BODY, item.COSTUME_TYPE_HAIR, ):
			return True
	
		elif item.GetItemType() == item.ITEM_TYPE_ARMOR and item.GetItemSubType() in (item.ARMOR_BODY, ):
			return True
...
def MakeTextLine(parent, text = "", lPos = (0, 0), lCenter = (True, True)):
	textLine = TextLine()
	textLine.SetParent(parent)
	textLine.SetText(text)
	textLine.SetPosition(*lPos)
	if (lCenter[0]):
		textLine.SetWindowHorizontalAlignCenter()
		textLine.SetHorizontalAlignCenter()
	if (lCenter[1]):
...
class SimplyWindow(Window):
	def __init__(self, layer, flags, width, height, initializeMethod = lambda *args : None, destroyMethod = lambda *args : None):
		Window.__init__(self, layer)
		self.children		= {}
		self.windowConfig	= {
			"size"	:	{
	...
	def GetObject(self, key, index = -1):
		...
		if isinstance(child, (tuple, list, set)):
			if index != -1:
				return child[index]
			return child
	...
	def SetOpenEvent(self, event):
		self.openEvent = __mem_func__(event)

	def SetCloseEvent(self, event):
		self.closeEvent = __mem_func__(event)
