...
class GuildWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.changeGradeNameDialog.gradeNameSlot.OnIMEReturn = self.OnChangeGradeName
			self.changeGradeNameDialog.gradeNameSlot.OnPressEscapeKey = self.changeGradeNameDialog.Close
			self.commentSlot = self.pageWindow["BOARD"].GetChild("CommentValue")
			self.commentSlot.OnIMEReturn = self.OnPostComment
	...
	def RefreshGuildMark(self):
		guildID = net.GetGuildID()
		self.largeMarkBox.SetIndex(guildID)
		self.largeMarkBox.UpdateRect()
		self.largeMarkBox.SetScale(3)
		self.largeMarkBox.Show()
	...
	def __OnSelectMark(self, markFileName):
		...
		threading.Timer(1, self.RefreshGuildMark).start() #rozjeba©©em klawiature... refresh mark ui load 1 sec bc sync
