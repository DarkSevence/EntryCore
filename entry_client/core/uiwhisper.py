...
class WhisperDialog(ui.ScriptWindow):
	...
	def ResizeWhisperDialog(self):
		...
		minWidth = 450  # ustaw minimalną szerokość
		minHeight = 200  # ustaw minimalną wysokość

		# Sprawdź, czy bieżąca pozycja przycisku resize jest mniejsza niż minimalne wymiary
		if xPos < minWidth - 20:
			self.resizeButton.SetPosition(minWidth - 20, yPos)
			xPos = minWidth - 20  # zaktualizuj xPos po zmianie pozycji przycisku resize
		if yPos < minHeight - 20:
			self.resizeButton.SetPosition(xPos, minHeight - 20)
			yPos = minHeight - 20  # zaktualizuj yPos po zmianie pozycji przycisku resize
