...
void CArea::__SetObjectInstance_SetBuilding(TObjectInstance * pObjectInstance, const TObjectData * c_pData, CProperty * pProperty)
{
	...
	if (!pThing)
	{
		TraceError("Brak modelu dla budynku: %s, CRC: %u. SprawdŸ, czy plik modelu istnieje.", Data.strFileName.c_str(), c_pData->dwCRC);
		return;
	}
...
{
	if (!c_szAttributeFileName || strlen(c_szAttributeFileName) == 0)
	{
		//TraceError("__LoadAttribute: Skipping .mdatr file loading");
		return;
	}
...
bool CArea::__Load_LoadAmbience(const char* c_szFileName)
{
	...
	for (DWORD i = 0; i < dwCount; ++i)
	{
		...
		// Dodano logowanie CRC i pozycji obiektu
		//TraceError("Processing object: %s, CRC: %u, Position: (%f, %f, %f)", szObjectName, ObjectData.dwCRC, ObjectData.Position.x, ObjectData.Position.y, ObjectData.Position.z);
