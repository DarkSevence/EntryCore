...
BOOL CItemData::LoadItemData(const char * c_szFileName)
{
	...
	// Attaching Data
	// Item żˇ Attaching Data ŔĎ´Ü ľřŔ˝.
...
//Ľ­ąöżÍ µżŔĎ Ľ­ąö ÇÔĽö şŻ°ć˝Ă °°ŔĚ şŻ°ć!!(ŔĚČÄżˇ ÇŐÄŁ´Ů)
//SocketCount = 1 ŔĚ¸é ĂĘ±Ţą«±â
//SocketCount = 2 ŔĚ¸é Áß±Ţą«±â
//SocketCount = 3 ŔĚ¸é °í±Ţą«±â
...
{
	// Logowanie wartości specular przed zwróceniem
	Tracef("Specular power (UINT): %u", m_ItemTable.bSpecular);
...
float CItemData::GetSpecularPowerf() const
{
	...
	float fSpecularPower = float(uSpecularPower) / 100.0f;

	// Logowanie przeliczonej wartości specular na float
	Tracef("Specular power (float): %f", fSpecularPower);
	return fSpecularPower;
}


//refine °ŞŔş ľĆŔĚĹŰąřČŁ łˇŔÚ¸®żÍ ŔĎÄˇÇŃ´Ů-_-(Ĺ×ŔĚşíŔĚżëŔ¸·Î ąŮ˛Ü żąÁ¤)
