...
bool CMapOutdoor::LoadSetting(bool bSnowTexture)
{
	std::string strFileName = GetMapDataDirectory() + "\\Setting.txt";
	...
	{
		TraceError("MapOutdoor::LoadSetting(c_szFileName=%s) - LoadMultipleTextData", strFileName.c_str());
		return false;
	}

	if (stTokenVectorMap.end() == stTokenVectorMap.find("textureset"))
	{
		TraceError("MapOutdoor::LoadSetting(c_szFileName=%s) - FIND 'textureset' - FAILED", strFileName.c_str());
		return false;
	}
	...
	else
	{
		if (bSnowTexture)
		{
			stTextureSetFileName = "textureset\\snow\\snow_" + stTextureSetFileName.substr(11);
		}
	}

	if (!m_TextureSet.Load(stTextureSetFileName.c_str(), m_fTerrainTexCoordBase))
	{
		TraceError("MapOutdoor::LoadSetting(c_szFileName=%s) - LOAD TEXTURE SET(%s) ERROR", strFileName.c_str(), stTextureSetFileName.c_str());
		return false;
	}
