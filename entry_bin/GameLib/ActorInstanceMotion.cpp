...
bool CActorInstance::InterceptMotion(EMotionPushType iMotionType, WORD wMotion, float fBlendTime, UINT uSkill, float fSpeedRatio)
{
	...
	{
		Tracenf("CActorInstance::InterceptMotion(iMotionType=%d, wMotion=%d, fBlendTime=%f) - GetMotionKey ERROR: m_wcurMotionMode=%d, wMotion=%d",
			iMotionType, wMotion, fBlendTime, m_wcurMotionMode, wMotion);
		return false;
	}
