...
{
	// Use a const variable to store the maximum map size value
	const short kMaxMapSize = MAX_MAPSIZE;

	// Use a single return statement at the end of the function
	if (sTerrainCountX == 0 || sTerrainCountX > kMaxMapSize || sTerrainCountY == 0 || sTerrainCountY > kMaxMapSize)
	{
		return false;
	}
...
void CMapOutdoor::SetEnvironmentLensFlare()
{
	...
	// Use a single boolean value to store the lens flare and main flare enable states
	bool lensFlareEnabled = mc_pEnvironmentData->bLensFlareEnable == 1;
	bool mainFlareEnabled = mc_pEnvironmentData->bMainFlareEnable == 1;

	m_LensFlare.CharacterizeFlare(lensFlareEnabled, mainFlareEnabled, mc_pEnvironmentData->fLensFlareMaxBrightness, mc_pEnvironmentData->LensFlareBrightnessColor);

	m_LensFlare.Initialize("d:/ymir work/environment");

	// Check if the main flare texture file name is not empty before calling SetMainFlare()
...
{
	// Use std::unique_ptr to manage the memory of the m_pTerrainPatchProxyList array
	auto patchProxyList = std::unique_ptr<CTerrainPatchProxy[]>{ m_pTerrainPatchProxyList };

	// Set m_pTerrainPatchProxyList to nullptr
...
{
	// Clear the area vector and delete vector
	m_AreaVector.clear();
	m_AreaDeleteVector.clear();

	// Free all the elements in the CArea pool
	CArea::ms_kPool.FreeAll();

	// Set all elements of m_pArea to nullptr
	std::fill(std::begin(m_pArea), std::end(m_pArea), nullptr);
}
...
{
	// Clear the terrain vector and delete vector
	m_TerrainVector.clear();
	m_TerrainDeleteVector.clear();

	// Free all the elements in the CTerrain pool
	CTerrain::ms_kPool.FreeAll();

	// Set all elements of m_pTerrain to nullptr
	std::fill(std::begin(m_pTerrain), std::end(m_pTerrain), nullptr);
}
...
bool CMapOutdoor::GetTerrainNum(int ix, int iy, BYTE* pbyTerrainNum)
{
	if (iy < 0)
		iy = -iy;

	const int kTerrainXSize = CTerrainImpl::TERRAIN_XSIZE;
	const int kTerrainYSize = CTerrainImpl::TERRAIN_YSIZE;

	WORD wTerrainNumX = ix / kTerrainXSize;
	WORD wTerrainNumY = iy / kTerrainYSize;
...
{
	// Use std::abs() to calculate the absolute value of fy
	fy = std::abs(fy);
	...
	usCoordX = (unsigned short)(lx / CTerrainImpl::TERRAIN_XSIZE);
	usCoordY = (unsigned short)(ly / CTerrainImpl::TERRAIN_YSIZE);
...
DWORD CMapOutdoor::GetShadowMapColor(float fx, float fy)
{
	...
	int terrainX = (int)(fXRef / fTerrainSize) + 1;
	int terrainY = (int)(fYRef / fTerrainSize) + 1;

	if (terrainX < 0 || terrainX > 2 || terrainY < 0 || terrainY > 2)
		return 0xFFFFFFFF;

	int terrainIndex = terrainX + terrainY * 3;
	CTerrain* pTerrain;
	if (GetTerrainPointer(terrainIndex, &pTerrain))
		return pTerrain->GetShadowMapColor(fXRef - (terrainX - 1) * fTerrainSize, fYRef - (terrainY - 1) * fTerrainSize);
	else
		return 0xFFFFFFFF;
}
