...
std::string ToLower(const char* str) 
{
	...
	while (*str != '\0') 
	{
		lowerCaseStr += static_cast<char>(std::tolower(*str));
		++str;
	}
	return lowerCaseStr;
	}
...
DWORD GetPropertyType(const char* c_szTypeName) {
	std::string typeNameLower = ToLower(c_szTypeName);
	for (DWORD i = 0; i < PROPERTY_TYPE_MAX_NUM; ++i) {
		if (typeNameLower == ToLower(c_szPropertyTypeName[i])) {
			return i;
		}
	}
...
bool PropertyBuildingDataToString(TPropertyBuilding * pData, CProperty * pProperty)
{
	...
	pProperty->PutString("IsAttributeData", IntegerNumberToString(pData->isAttributeData));
...
{
	// Wczytywanie podstawowych w³aœciwoœci...
	const char* c_pszPropertyType;
	const char* c_pszPropertyName;
	...
	//TraceError("Loading building file: %s", pData->strFileName.c_str());

	pData->isAttributeData = true;
	const char* c_pszIsAttributeData;
	if (pProperty->GetString("IsAttributeData", &c_pszIsAttributeData) && strcmp(c_pszIsAttributeData, "0") == 0)
	{
		pData->isAttributeData = false;
		//TraceError("Attribute data file (.mdatr) will not be loaded for: %s, as IsAttributeData is set to 0", pData->strFileName.c_str());
	}
	else
	{
		pData->strAttributeDataFileName = CFileNameHelper::NoExtension(pData->strFileName) + ".mdatr";
		//TraceError("Loading attribute data file (.mdatr): %s", pData->strAttributeDataFileName.c_str());
	}
