...
bool CActorInstance::TestActorCollision(CActorInstance& targetActor)
{
...
#if defined(DISABLE_SHOP_COLLISION_DETECTION)
	if (IsShop(targetActor))
	{
		return false;
	}
#endif

	D3DXVECTOR3 vectorDistance = D3DXVECTOR3(targetActor.m_x - m_x, targetActor.m_y - m_y, targetActor.m_z - m_z);
	float squaredDistance = D3DXVec3LengthSq(&vectorDistance);

	if (squaredDistance > 640000.0f)
	{
		return false;
	}

	TCollisionPointInstanceList* mainActorCollisionPoints;
	TCollisionPointInstanceList* targetActorCollisionPoints;

	if (isAttacking() || IsWaiting())
	{
		mainActorCollisionPoints = &m_DefendingPointInstanceList;
		targetActorCollisionPoints = &targetActor.m_DefendingPointInstanceList;
	}
	else
	{
		mainActorCollisionPoints = &m_BodyPointInstanceList;
		targetActorCollisionPoints = &targetActor.m_BodyPointInstanceList;
	}

	for (auto mainPointIt = mainActorCollisionPoints->begin(); mainPointIt != mainActorCollisionPoints->end(); ++mainPointIt)
	{
		for (auto targetPointIt = targetActorCollisionPoints->begin(); targetPointIt != targetActorCollisionPoints->end(); ++targetPointIt)
		{
			const auto& mainSphereVector = mainPointIt->SphereInstanceVector;
			const auto& targetSphereVector = targetPointIt->SphereInstanceVector;

			for (size_t i = 0; i < mainSphereVector.size(); ++i)
			{
				for (size_t j = 0; j < targetSphereVector.size(); ++j)
				{
					const auto& mainSphere = mainSphereVector[i];
					const auto& targetSphere = targetSphereVector[j];

					if (DetectCollisionDynamicSphereVSDynamicSphere(mainSphere, targetSphere))
					{
						if (GetVector3Distance(mainSphere.v3Position, targetSphere.v3Position) <= GetVector3Distance(mainSphere.v3LastPosition, targetSphere.v3Position))
						{
							return true;
						}
