...
BOOL CRaceData::LoadRaceData(const char * c_szFileName)
{
	...
	if (TextFileLoader.SetChildNode("hairdata"))
	{
		...
		if (TextFileLoader.GetTokenString("pathname", &strPathName))
		{
			...
			while (TextFileLoader.SetChildNode("hairdata", dwHairDataCount))
			{
				...
				DWORD dwHairIndex;
				if (!TextFileLoader.GetTokenDoubleWord("hairindex", &dwHairIndex))
				{
					TextFileLoader.SetParentNode();
					continue;
				}
				
				std::string strModel, strSourceSkin, strTargetSkin;
				...
				SetHairSkin(dwHairIndex, 0, (strPathName + strModel).c_str(), (strPathName + strSourceSkin).c_str(), (strPathName + strTargetSkin).c_str());
	...
	if (TextFileLoader.SetChildNode("costumedata"))
	{
		std::string strPathName;
		DWORD dwCostumeDataCount = 0;
		if (TextFileLoader.GetTokenString("pathname", &strPathName))
		{
			while (TextFileLoader.SetChildNode("costumedata", dwCostumeDataCount))
			{

				TextFileLoader.GetTokenString("specialpath", &strPathName);

				DWORD dwCostumeIndex;
				if (!TextFileLoader.GetTokenDoubleWord("costumeindex", &dwCostumeIndex))
				{
					TextFileLoader.SetParentNode();
					continue;
				}
				...
				if (TextFileLoader.GetTokenString("model", &strModel))
				{
					SetShapeModel(dwCostumeIndex, (strPathName + strModel).c_str());
				}
				else
				{
					if (!TextFileLoader.GetTokenString("local_model", &strModel))
					...
					SetShapeModel(dwCostumeIndex, strModel.c_str());
				}
				...
				if (TextFileLoader.GetTokenString("local_sourceskin", &strSourceSkin) &&
					TextFileLoader.GetTokenString("local_targetskin", &strTargetSkin))
				{
					AppendShapeSkin(dwCostumeIndex, 0, strSourceSkin.c_str(), strTargetSkin.c_str());
				}

				if (TextFileLoader.GetTokenString("sourceskin", &strSourceSkin) &&
					TextFileLoader.GetTokenString("targetskin", &strTargetSkin))
				{
					AppendShapeSkin(dwCostumeIndex, 0, (strPathName + strSourceSkin).c_str(), (strPathName + strTargetSkin).c_str());
				}
