...
bool CPropertyManager::Register(const char* c_pszFileName, CProperty** ppProperty)
{
	...
	if (!CEterPackManager::Instance().Get(file, c_pszFileName, &c_pvData))
	{
		//TraceError("[ERROR] Nie mo¿na za³adowaæ property z pliku: %s", c_pszFileName);
		return false;
	}
	...
	{
		//TraceError("[ERROR] Nie uda³o siê przeczytaæ property z pamiêci dla pliku: %s", c_pszFileName);
		delete pProperty;
		return false;
	}
	...
	{
		Tracef("[WARN] Property ju¿ zarejestrowane, zastêpowanie %s przez %s",
...
bool CPropertyManager::Erase(DWORD dwCRC)
{
	...
	if (!m_isFileMode)	// ÆÄÀÏ ¸ðµå°¡ ¾Æ´Ï¸é ÆÑ¿¡¼­µµ Áö¿ò
