...
class CActorInstance : public IActorInstance, public IFlyTargetableObject
{
	...
	public:
		...
		void SetGuildMarkImage(const char* imagePath, int index);
		...
		bool IsShop(CActorInstance& actor);
		#endif
	...
	public:
		bool TestActorCollision(CActorInstance& targetActor);
		bool TestPhysicsBlendingCollision(CActorInstance& targetActor);
	...
	protected:
		...
		float CalculateAdjustedInvisibilityDuration(const uint32_t uiSkill, const NRaceData::TAttackData& c_rAttackData);
	...
	protected:
		...
		private:
			float m_fHeight; // Dodanie zmiennej do przechowywania wysoko쐁i instancji
