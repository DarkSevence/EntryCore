...
bool CMapManager::LoadMap(const std::string c_rstrMapName, float x, float y, float z)
{
	...
	{
		TraceError(static_cast<std::string>("CMapManager::LoadMap() Indoor Map Load Failed").c_str());
		return false;
	}
	else if (CMapBase::MAPTYPE_OUTDOOR == rkMap.GetType())
	{
		if (!rkMap.Load(x, y, z))
		{
			TraceError(static_cast<std::string>("CMapManager::LoadMap() Outdoor Map Load Failed").c_str());
			return false;
		}
	...
	{
		TraceError(static_cast<std::string>("CMapManager::LoadMap() Invalid Map Type").c_str());
		return false;
	}
...
#if defined(ENABLE_ENVIRONMENT_EFFECT_OPTION)
	if (m_bSnowTexture && CPythonBackground::Instance().IsSnowMap(c_rstrMapName))
	{
		m_pkMap->LoadSetting(true);
	}
#endif
...
void CMapManager::BeginEnvironment()
{
	...
	// Fog
	STATEMANAGER.SaveRenderState(D3DRS_FOGENABLE, m_isFogModeEnabled ? mc_pcurEnvironmentData->bFogEnable : false);
...
#if defined(ENABLE_ENVIRONMENT_EFFECT_OPTION)
void CMapManager::SetSnowTexture(bool bEnable)
{
	if (m_pkMap)
	{
		m_pkMap->LoadSetting(bEnable);
	}

	m_bSnowTexture = bEnable;
}
#endif
