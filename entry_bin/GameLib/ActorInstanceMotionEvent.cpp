...
void CActorInstance::MotionEventProcess()
{
	...
	// Use a local variable to store the motion event data count,
	// to avoid multiple calls to GetMotionEventDataCount().
	const DWORD motionEventDataCount = m_pkCurRaceMotionData->GetMotionEventDataCount();

	// Use a vector to store the motion event data pointers,
	// to avoid multiple calls to GetMotionEventDataPointer().
	std::vector<const CRaceMotionData::TMotionEventData*> motionEventData;
	motionEventData.reserve(motionEventDataCount);
	for (DWORD i = 0; i < motionEventDataCount; ++i)
	{
		const CRaceMotionData::TMotionEventData* c_pData;
		if (m_pkCurRaceMotionData->GetMotionEventDataPointer(i, &c_pData))
			motionEventData.push_back(c_pData);
	}

	// Use a loop to process the motion event data in parallel using OpenMP.
	// This will allow the function to utilize multiple CPU cores,
	// potentially improving performance.
#pragma omp parallel for
	for (DWORD i = 0; i < motionEventData.size(); ++i)
	{
		MotionEventProcess(m_kCurMotNode.dwcurFrame, i, motionEventData[i]);
	}
}
