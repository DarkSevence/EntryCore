...
constexpr int32_t SHOP_RACE_ID_MIN = 30000;
constexpr int32_t SHOP_RACE_ID_MAX = 30001;

bool CActorInstance::IsShop(CActorInstance& actor)
{
	int32_t actorRace = actor.GetRace();
	return actorRace >= SHOP_RACE_ID_MIN && actorRace <= SHOP_RACE_ID_MAX;
}
#endif
...
float CActorInstance::GetHeight()
{
	...
	if (fRaceHeight == 0.0f)
	{
		//TraceError("CActorInstance::GetHeight() - Height for race %lu is initially 0.0f", dwRace);
		fRaceHeight = CGraphicThingInstance::GetHeight();
		if (fRaceHeight == 0.0f) {
			//TraceError("CActorInstance::GetHeight() - CGraphicThingInstance::GetHeight() also returned 0.0f for race %lu", dwRace);
		}
		CRaceManager::instance().SetRaceHeight(dwRace, fRaceHeight);
	}

	//TraceError("CActorInstance::GetHeight() - Height for race %lu is %f", dwRace, fRaceHeight);
