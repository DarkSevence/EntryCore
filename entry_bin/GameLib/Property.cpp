...
{
	const char* pcData = static_cast<const char*>(c_pvData);

	if (*reinterpret_cast<const DWORD*>(pcData) != MAKEFOURCC('Y', 'P', 'R', 'T'))
	{
		//TraceError("[ERROR] Niepoprawny nag³ówek FourCC w pliku: %s", c_pszFileName);
		return false;
	}
	...
	{
		//TraceError("[ERROR] Niepoprawny format pliku po FourCC: %s", c_pszFileName);
		return false;
	}
	...
	if (m_stCRC.empty())
	{
		///TraceError("[ERROR] Pusta linia CRC w pliku: %s", c_pszFileName);
		return false;
	}

	m_dwCRC = atoi(m_stCRC.c_str());
	//TraceError("[INFO] Odczytano CRC: %u dla pliku: %s", m_dwCRC, c_pszFileName);

	CTokenVector stTokenVector;
	for (DWORD i = 1; i < textFileLoader.GetLineCount(); ++i)
	{
		stTokenVector.clear(); // Zapewnienie, ¿e wektor jest pusty przed kolejnym u¿yciem
		if (!textFileLoader.SplitLine(i, &stTokenVector) || stTokenVector.size() < 2)
		{
			//TraceError("[ERROR] Nie mo¿na przetworzyæ linii: %d w pliku: %s. Problem z formatem linii lub jej zawartoœci¹.", i + 1, c_pszFileName);
			continue; 
		}
