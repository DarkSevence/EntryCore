...
{
	auto is_visible_part = IsVisiblePart(PART_OBJECT);
	...
	{
		CArea* pArea;
		auto got_area_pointer = GetAreaPointer(i, &pArea);
		if (got_area_pointer)
		{
			pArea->RenderEffect();
		}
...
struct CMapOutdoor_LessThingInstancePtrRenderOrder
{
	bool operator() (CGraphicThingInstance* pkLeft, CGraphicThingInstance* pkRight)
	{
		...
		D3DXVECTOR3 vv = c_rv3CameraPos - c_v3RightPos;
		D3DXVECTOR3 vv2 = c_rv3CameraPos - c_v3LeftPos;
...
{
	assert(static_cast<CTerrainPatchProxy*>(m_pTerrainPatchProxyList) != nullptr && "CMapOutdoor::__SetPatchDrawVector");
	...
	for (const auto& patch_info : m_PatchVector)
	{
		auto pTerrainPatchProxy = &m_pTerrainPatchProxyList[patch_info.second];

		if (!pTerrainPatchProxy->isUsed())
			continue;

		const auto patch_num = pTerrainPatchProxy->GetPatchNum();
		if (patch_num < 0)
			continue;

		const auto terrain_num = pTerrainPatchProxy->GetTerrainNum();
		if (terrain_num == 0xFF)
			continue;

		if (!pTerrain || pTerrainPatchProxy->GetTerrainNum() != terrain_num)
		{
			if (!GetTerrainPointer(terrain_num, &pTerrain))
				continue;
		}

		aPatchDrawStruct.fDistance = patch_info.first;
		aPatchDrawStruct.byTerrainNum = terrain_num;
		aPatchDrawStruct.lPatchNum = patch_num;
