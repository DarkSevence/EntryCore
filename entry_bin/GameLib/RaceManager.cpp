...
bool IsGuildRace(uint32_t race)
{
	bool isGuildRace = false;

	if ((race >= CRaceManager::GUILD_RACE_MIN && race < CRaceManager::GUILD_RACE_MAX) || (CRaceManager::SPECIAL_GUILD_RACE == race))
	{
		isGuildRace = true;
	}
...
bool IsNPCRace(uint32_t race)
{
	bool isNPCRace = false;

	if (race > CRaceManager::NPC_RACE_START_RANGE)
	{
		isNPCRace = true;
	}
...
bool IsMonsterRace(uint32_t race)
{
	// Now it correctly checks the race within the exclusive range for monster, including Metin races as monsters.
	return (race >= CRaceManager::MONSTER_RACE_MIN && race < CRaceManager::MONSTER2_RACE_MIN) ||
		(race >= CRaceManager::MONSTER_RACE_METIN_MIN && race <= CRaceManager::MONSTER_RACE_METIN_MAX); // Include Metin races in monster
}


bool IsMonster2Race(uint32_t race)
{
	// Adjusted the conditions to correctly identify races that should load from 'monster2'.
	return (race >= CRaceManager::MONSTER2_RACE_MIN && race <= CRaceManager::MONSTER2_RACE_MAX) ||
		race == CRaceManager::MONSTER2_RACE_SPECIAL1 ||
		race == CRaceManager::MONSTER2_RACE_SPECIAL2 ||
		race >= CRaceManager::MONSTER2_RACE_START_RANGE;
}


void GetRaceResourcePathes(uint32_t race, std::vector<std::string>& vec_stPathes)
{
	if (IsNPCRace(race))
	{
		if (race >= CRaceManager::MIN_PET_NPC_RACE && race <= CRaceManager::MAX_PET_NPC_RACE)
		{
			vec_stPathes.emplace_back(CRaceManager::NPC_PET_PATH);
		}
		else if (race >= CRaceManager::MIN_MOUNT_NPC_RACE && race <= CRaceManager::MAX_MOUNT_NPC_RACE)
		{
			vec_stPathes.emplace_back(CRaceManager::NPC_MOUNT_PATH);
		}
		else
		{
			vec_stPathes.emplace_back(CRaceManager::NPC_PATH);
		}
	}
	else if (IsMonsterRace(race))
	{
		vec_stPathes.emplace_back(CRaceManager::MONSTER_PATH);
	}
	else if (IsMonster2Race(race))
	{
		vec_stPathes.emplace_back(CRaceManager::MONSTER2_PATH);
	}
	else if (IsGuildRace(race))
	{
		vec_stPathes.emplace_back(CRaceManager::GUILD_PATH);
	}
}
...
CRaceData* CRaceManager::__LoadRaceData(DWORD dwRaceIndex)
{
	...
	GetRaceResourcePathes(dwRaceIndex, vec_stFullPathName);
