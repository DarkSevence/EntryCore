...
{
	auto& cooldownMap = m_CoolDownStore[dwKey];
	auto cooldownIterator = cooldownMap.find(dwSlotIndex);

	if (cooldownIterator != cooldownMap.end())
	{
		auto& storeCooldown = cooldownIterator->second;
		storeCooldown.fCoolTime = fCoolTime;
		storeCooldown.fElapsedTime = CTimer::Instance().GetCurrentSecond() - fElapsedTime;
		storeCooldown.bActive = false;
	}
	else
	{
		SStoreCoolDown newCooldown{};
		newCooldown.fCoolTime = fCoolTime;
		newCooldown.fElapsedTime = CTimer::Instance().GetCurrentSecond() - fElapsedTime;
		newCooldown.bActive = false;
		cooldownMap.emplace(dwSlotIndex, newCooldown);
	}
}
...
void CSlotWindow::RestoreSlotCoolTime(DWORD dwKey)
{
	...
	{
		TSlot* pSlot;
		if (!GetSlotPointer(cooldownPair.first, &pSlot))
		{
			return;
		}

		pSlot->fCoolTime = cooldownPair.second.fCoolTime;
		pSlot->fStartCoolTime = cooldownPair.second.fElapsedTime;

		if (cooldownPair.second.bActive)
		{
			ActivateSlot(cooldownPair.first);
		}
		else
		{
			DeactivateSlot(cooldownPair.first);
		}
	}
}
...
{
	auto& cooldownStore = m_CoolDownStore[CPythonSkill::SKILL_TYPE_ACTIVE];

	auto it = cooldownStore.find(dwIndex1);

	if (it != cooldownStore.end())
	{
		TSlot* pSlot1;
		...
		auto& slotCooldown = it->second;
		cooldownStore[dwIndex2] = slotCooldown;
		cooldownStore.erase(dwIndex1);
