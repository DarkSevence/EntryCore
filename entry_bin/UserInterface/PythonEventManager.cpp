...
void CPythonEventManager::ProcessEventSet(TEventSet * pEventSet)
{
	...
	switch (iEventType)
	{
		...
		{
			//ÄłĆ˝ĹÍ ¸¸µé±â Á¦±ą Ľł¸íµîżˇĽ­ Ľł¸í ąŘŔ¸·Î ĂÄÁö´Â ąö±× ĽöÁ¤/EVENT_TYPE_WAIT °ü·ĂÇŘĽ­ Ĺ×˝şĆ® ÇĘżä.
			pEventSet->iyLocal = 0;
			pEventSet->isLock = true;
			break;
		}
		...
		case EVENT_TYPE_QUEST_BUTTON:
		{
			...
			// Äů˝şĆ® UI ¸®´şľóŔĚ µÇ¸é ÇŘ°á µÇąÇ·Î ŔĎ´Ü żëČĄĽ®¸¸ ¶«»§ by chrislee
			if (!strcmp(name, "Á¶°˘ł­ żëČĄĽ®?....")) {
...
void CPythonEventManager::SetFontColor(int32_t iIndex, float fRed, float fGreen, float fBlue, float fAlpha)
{
	if (!CheckEventSetIndex(iIndex))
	{
		return;
	...
	auto pEventSet = m_EventSetVector[iIndex];
	...
	if (pEventSet->pCurrentTextLine)
	{
		...
		pEventSet->CurrentColor.a = fAlpha;
		pEventSet->pCurrentTextLine->SetColor(pEventSet->CurrentColor.r, pEventSet->CurrentColor.g, pEventSet->CurrentColor.b, pEventSet->CurrentColor.a);
	}
}

CPythonEventManager::CPythonEventManager()
	: m_poInterface(0), m_strLeftTimeString("ł˛Ŕş ˝Ă°Ł : %dĂĘ")
{
