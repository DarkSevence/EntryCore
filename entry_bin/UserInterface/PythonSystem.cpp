...
void CPythonSystem::SaveWindowStatus(int index, int isVisible, int isMinimized, int xPos, int yPos, int height)
{
	auto& windowStatus = m_WindowStatus[index];

	windowStatus.isVisible = isVisible;
	windowStatus.isMinimized = isMinimized;
	windowStatus.xPosition = xPos;
	windowStatus.yPosition = yPos;
...
void CPythonSystem::SetCameraMode(uint8_t bMode)
{
	m_Config.bCameraMode = bMode;
}

uint8_t CPythonSystem::GetCameraMode() const
{
...
void CPythonSystem::SetShowMobAIFlagFlag(bool iFlag)
{
...
void CPythonSystem::SetShowMobLevelFlag(bool iFlag)
{
...
#if defined(WJ_SHOW_MOB_INFO)
	#if defined(ENABLE_SHOW_MOBAIFLAG)
	fprintf(fp, "SHOW_MOBAIFLAG\t%d\n", static_cast<int>(m_Config.bShowMobAIFlag));
	#endif
	#if defined(ENABLE_SHOW_MOBLEVEL)
	fprintf(fp, "SHOW_MOBLEVEL\t%d\n", static_cast<int>(m_Config.bShowMobLevel));
	#endif
#endif
