...
bool CPythonMiniMap::LoadDisabledAtlasConfig(const std::string& configPath)
{
	CMappedFile File;
	LPCVOID pData;

	if (!CEterPackManager::Instance().Get(File, configPath.c_str(), &pData))
	{
		TraceError("Failed to load or open JSON configuration file: %s", configPath.c_str());
		return false;
	}

	size_t fileSize = File.Size();
	std::string fileContent((char*)pData, fileSize);

	try
	{
		auto json = nlohmann::json::parse(fileContent);
		m_DisabledAtlases.clear();
		for (const auto& entry : json["disabled_atlases"])
		{
			m_DisabledAtlases.insert(entry.get<std::string>());
		}
	}
	catch (const nlohmann::json::exception& e)
	{
		TraceError("JSON parse error: %s", e.what());
		return false;
	}
...
bool CPythonMiniMap::LoadAtlas()
{
	...
	CMapOutdoor& rkMap = rkBG.GetMapOutdoorRef();
	std::string mapName = rkMap.GetName();

	if (m_DisabledAtlases.find(mapName) != m_DisabledAtlases.end())
	{
		return false;
	}
	...
	{
		TraceError("CPythonMiniMap::LoadAtlas - Failed to load atlas file: %s", atlasFileName);
	}
