...
//Ĺ×ŔĚşíżˇĽ­ ŔĚ¸§ŔĚ "." ŔÎ °Íµé
//Â÷ČÄżˇ Ľ­ąöżˇĽ­ ş¸ł»ÁÖÁö ľĘ°Ô µÇ¸é ľřľîÁú ÇÔĽö..(Ľ­ąö´Ô˛˛ ˛Ŕ!!ÇůąÚ; )
...
bool CPythonNetworkStream::RecvCharacterAppendPacket()
{
	...
	kNetActorData.m_stName="";
	kNetActorData.m_kAffectFlags.CopyData(0, sizeof(chrAddPacket.playerActiveAffects[0]), &chrAddPacket.playerActiveAffects[0]);
	kNetActorData.m_kAffectFlags.CopyData(32, sizeof(chrAddPacket.playerActiveAffects[1]), &chrAddPacket.playerActiveAffects[1]);
...
bool CPythonNetworkStream::RecvCharacterAdditionalInfo()
{
	...
	{
		TraceError("RecvCharacterAdditionalInfo name=%s vid=%d race=%d Error",chrInfoPacket.name,chrInfoPacket.dwVID,kNetActorData.m_dwRace);
	}
	return true;
}
...
{
	TraceError("TPacketGCCharacterAdd2´Â ľ˛Áö ľĘ´Â ĆĐĹ¶ŔÔ´Ď´Ů.");
...
bool CPythonNetworkStream::RecvCharacterUpdatePacket()
{
	...
	kNetUpdateActorData.m_kAffectFlags.CopyData(0, sizeof(chrUpdatePacket.playerActiveAffects[0]), &chrUpdatePacket.playerActiveAffects[0]);
	kNetUpdateActorData.m_kAffectFlags.CopyData(32, sizeof(chrUpdatePacket.playerActiveAffects[1]), &chrUpdatePacket.playerActiveAffects[1]);
...
{
	// NOTE : Ä«¸Ţ¶ó°ˇ ¶Ążˇ ąŻČ÷´Â ą®Á¦ŔÇ ÇŘ°áŔ» Ŕ§ÇŘ ¸ŢŔÎ Äł¸ŻĹÍ°ˇ ÁöÇüżˇ żĂ·ÁÁö±â
	//        Ŕüżˇ ¸ĘŔ» ľ÷µĄŔĚĆ® ÇŘ łôŔĚ¸¦ ±¸ÇŇ Ľö ŔÖµµ·Ď ÇŘłőľĆľß ÇŐ´Ď´Ů.
	//        ´Ü, °ÔŔÓŔĚ µéľî°Ą¶§°ˇ ľĆ´Ń ŔĚąĚ Äł¸ŻĹÍ°ˇ Ăß°ˇ µČ ŔĚČÄżˇ¸¸ ÇŐ´Ď´Ů.
	//        ÇĺµĄ ŔĚµżŔÎµĄ żÖ Move·Î ľČÇĎ°í Append·Î ÇĎ´ÂÁö..? - [levites]
	...
	if (rkPlayer.IsMainCharacterIndex(pkNetActorData->m_dwVID))
	{
		...
		if (rkPlayer.NEW_GetMainActorPtr())
		{
			...
			// NOTE : »ç±Í Ĺ¸żöŔĎ °ćżě GOTO ·Î ŔĚµż˝Ăżˇµµ ¸Ę ŔĚ¸§Ŕ» Ăâ·ÂÇĎµµ·Ď Ăł¸®
			{
...
bool CPythonNetworkStream::RecvCharacterDeletePacket()
{
	...
	// Äł¸ŻĹÍ°ˇ »ç¶óÁú¶§ °łŔÎ »óÁˇµµ ľřľÖÁÝ´Ď´Ů.
	// Key Check ¸¦ ÇĎ±â¶§ą®żˇ ľřľîµµ »ó°üŔş ľř˝Ŕ´Ď´Ů.
