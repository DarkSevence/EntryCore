...
void CPythonTextTail::RegisterChatTail(DWORD VirtualID, const char* c_szChat)
{
	...
	bool isMounting = pCharacterInstance->IsMountingHorse();
	float baseHeight = pCharacterInstance->GetBaseHeight();
	float scaledHeight = pCharacterInstance->ScaleTextTailHeight(baseHeight, isMounting);
	float heightToAdd;

	if (isMounting) {
		if (baseHeight > 160.0f) {
			heightToAdd = 250.0f;
		}
		else {
			heightToAdd = 205.0f;
		...
		heightToAdd = pCharacterInstance->GetGraphicThingInstanceRef().GetHeight() + 10.0f;
	}
...
//	TraceError("RegisterChatTail - VirtualID: %lu, Mounting: %d, Base Height: %f, Scaled Height: %f, Total Added Height: %f", VirtualID, isMounting, baseHeight, scaledHeight, heightToAdd);
...
void CPythonTextTail::SetItemTextTailOwner(DWORD dwVID, const char * c_szName)
{
	...
	if (strlen(c_szName) > 0)
	{
		...
		pTextTail->pOwnerTextInstance->SetColor(1.0f, 2.0f, 0.0f);
		...
		if (pInstanceBase)
		{
			...
			{
				pTextTail->pOwnerTextInstance->SetColor(1.0f, 0.0f, 1.0f);
			}
		}
...
void CPythonTextTail::RefreshMark(DWORD dwVID)
{
	TTextTailMap::iterator itor = m_CharacterTextTailMap.find(dwVID);
	if (m_CharacterTextTailMap.end() == itor)
		return;
	...
	DWORD dwGuildID = CPythonCharacterManager::Instance().GetInstancePtr(dwVID)->GetGuildID();
	if (dwGuildID)
	{
		pTextTail->pMarkInstance = CGraphicMarkInstance::New();
		DWORD dwMarkID = CGuildMarkManager::Instance().GetMarkID(dwGuildID);
...
void CPythonTextTail::RefreshAllMarks()
{
	for (auto& pair : m_CharacterTextTailMap)
	{
		DWORD dwVID = pair.first;
		RefreshMark(dwVID);
	}
}
