...
void CPythonNetworkStream::GamePhase()
{
	...
	if (m_isRefreshGuildWndBoardPage)
	{
		...
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RefreshCommentsPage", Py_BuildValue("()"));
		s_nextRefreshTime = curTime + 300;
	}
...
bool CPythonNetworkStream::RecvChatPacket()
{
	...
	if (kChat.bCanFormat && CHAT_TYPE_SHOUT != kChat.type) 
	{
		...
		PythonLocaleManager::Instance().FormatLocaleString(sFormat);
		strncpy(buf, sFormat.c_str(), sizeof(buf));
		buf[sizeof(buf) - 1] = '\0';

		PythonLocaleManager::Instance().SplitIntoMultipleLines(sFormat, vecMultiLine);
	}
...
bool CPythonNetworkStream::RecvWhisperPacket()
{
	...
	if (whisperPacket.wSize - sizeof(whisperPacket) >= 512)
	{
		TraceError("Whisper packet size error: actual size exceeds buffer - %d", whisperPacket.wSize - sizeof(whisperPacket));
		return false;
	}
	...
	{
		PythonLocaleManager::Instance().FormatLocaleString(buf, sizeof(buf));
	}
...
bool CPythonNetworkStream::RecvExchangePacket()
{
	...
	case EXCHANGE_SUBHEADER_GC_START:
		...
		CPythonExchange::Instance().SetPlayerSelfName(CPythonPlayer::Instance().GetName());

		{
			CInstanceBase* pCharacterInstance = CPythonCharacterManager::Instance().GetInstancePtr(exchange_packet.arg1);

			if (pCharacterInstance)
				CPythonExchange::Instance().SetPlayerTargetName(pCharacterInstance->GetNameString());
...
bool CPythonNetworkStream::RecvQuestInfoPacket()
{
	...
	if (0 != (c_rFlag & QUEST_SEND_TITLE))
	{
		...
		szTitle[30]='\0';
		PythonLocaleManager::Instance().FormatLocaleString(szTitle, sizeof(szTitle));
	}
	if (0 != (c_rFlag & QUEST_SEND_CLOCK_NAME))
	{
		...
		szClockName[16]='\0';
		PythonLocaleManager::Instance().FormatLocaleString(szClockName, sizeof(szClockName));
	}
	...
	if (0 != (c_rFlag & QUEST_SEND_COUNTER_NAME))
	{
		...
		szCounterName[16]='\0';
		PythonLocaleManager::Instance().FormatLocaleString(szCounterName, sizeof(szCounterName));
	}
...
bool CPythonNetworkStream::RecvQuestConfirmPacket()
{
	...
	PythonLocaleManager::Instance().FormatLocaleString(kQuestConfirmPacket.msg, sizeof(kQuestConfirmPacket.msg));
...
bool CPythonNetworkStream::SendExchangeItemAddPacket(TItemPos ItemPos, BYTE byDisplayPos, bool isRightClickAutoMoveEnabled)
{
	if (!__CanActMainInstance())
	{
		return true;
	...
	packet.isRightClickAutoMoveEnabled = isRightClickAutoMoveEnabled;
...
bool CPythonNetworkStream::HandleQuestDestroyCommand(const std::string& str)
{
    static const std::string DESTROY_ALL_COMMAND = "[DESTROY_ALL]";
	
    if (str.compare(0, DESTROY_ALL_COMMAND.size(), DESTROY_ALL_COMMAND) == 0)
    {
...
bool CPythonNetworkStream::RecvScriptPacket()
{
    ...
    if (HandleQuestDestroyCommand(str)) 
    {
        return true;
    }

	PythonLocaleManager::Instance().FormatLocaleString(str);
	...
	switch(GuildPacket.subheader)
	{
		...
		case GUILD_SUBHEADER_GC_GRADE:
		{
			...
			for (BYTE i = 0; i < byCount; ++ i)
			{
				...
				PythonLocaleManager::Instance().FormatLocaleString(GradePacket.grade_name, sizeof(GradePacket.grade_name));
		...
		case GUILD_SUBHEADER_GC_GRADE_NAME:
		{
			...
			PythonLocaleManager::Instance().FormatLocaleString(szGradeName, sizeof(szGradeName));
		...
		case GUILD_SUBHEADER_GC_CHANGE_MEMBER_GRADE:
		{
			...
			TraceError(" <ChangeMemberGrade> %d, %d\n", dwPID, byGrade);
			__RefreshGuildWindowMemberPage();
			break;
		}
		...
		case GUILD_SUBHEADER_GC_GUILD_INVITE:
		{
			...
			if (!Recv(CHARACTER_NAME_MAX_LEN + 1, &szInviterName))
				return false;
...
bool CPythonNetworkStream::RecvAffectAddPacket()
{
	...
	TPacketAffectElement & rkElement = kAffectAdd.element;
	if (rkElement.applyOn == POINT_ENERGY)
	{
		CPythonPlayer::instance().SetStatus (POINT_ENERGY_END_TIME, CPythonApplication::Instance().GetServerTimeStamp() + rkElement.duration);
		__RefreshStatus();
	}
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_NEW_AddAffect", Py_BuildValue("(iiii)", rkElement.type, rkElement.applyOn, rkElement.applyValue, rkElement.duration));
...
bool CPythonNetworkStream::RecvAffectRemovePacket()
{
	...
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_NEW_RemoveAffect", Py_BuildValue("(ii)", kAffectRemove.type, kAffectRemove.applyOn));
...
bool CPythonNetworkStream::RecvTargetCreatePacket()
{
	...
	PythonLocaleManager::Instance().FormatLocaleString(kTargetCreate.szTargetName, sizeof(kTargetCreate.szTargetName));
...
bool CPythonNetworkStream::RecvGuildMarkUpdatePacket()
{
