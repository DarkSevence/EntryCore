...
class CPythonApplication : public CMSApplication, public CInputKeyboard, public IAbstractApplication
{
	...
	public:
		CPythonApplication();
		virtual~CPythonApplication() = default;
	...
	public:
		...
		void RenderGame();
		void PreRenderOperations();
		void RenderEnvironmentAndCharacters();
		void UpdateGame();
		void UpdateScreenSettings();
		void UpdateManagers(const POINT& ptMouse);
		...
		uint32_t CalculateWindowWidth(uint32_t clientWidth);
		uint32_t CalculateWindowHeight(uint32_t clientHeight);	
	...
	protected:
		...
		void SetFullScreenWindow(HWND hWnd, uint32_t dwWidth, uint32_t dwHeight, uint32_t dwBPP);
	...
	protected:
		...
		PythonLocaleManager			m_pyLocale;
	...
	protected:
		...
		BOOL						m_isWindowFullScreenEnabled;
