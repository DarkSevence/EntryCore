...
void CInstanceBase::StartWalking()
{
	...
	{
		m_adwCRCAffectEffect[AFFECT_GYEONGGONG] = EffectContainerAttachEffect(EFFECT_AFFECT_GYEONGGONG);
	}
	else if (IsAffect(AFFECT_KWAESOK))
	{
		m_adwCRCAffectEffect[AFFECT_KWAESOK] = EffectContainerAttachEffect(EFFECT_AFFECT_KWAESOK);
	}
}
...
void CInstanceBase::EndWalking(float fBlendingTime)
{
	...
	if (IsWalking()||!IsAttacked())
	{
		...
		if (IsAffect(AFFECT_GYEONGGONG))
		{
			EffectContainerDetachEffect(EFFECT_AFFECT_GYEONGGONG);
		}
		else if (IsAffect(AFFECT_KWAESOK))
		{
			EffectContainerDetachEffect(EFFECT_AFFECT_KWAESOK);
		}
	}
}
