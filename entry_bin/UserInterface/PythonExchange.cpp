...
void CPythonExchange::SetPlayerSelfName(std::string_view name)
{
	const auto len = min(name.size(), static_cast<std::size_t>(CHARACTER_NAME_MAX_LEN));
	name.copy(selfName.data(), len);
	selfName[len] = '\0';
}

void CPythonExchange::SetPlayerTargetName(std::string_view name)
{
	const auto len = min(name.size(), static_cast<std::size_t>(CHARACTER_NAME_MAX_LEN));
	name.copy(targetName.data(), len);
	targetName[len] = '\0';
}

std::string_view CPythonExchange::GetPlayerSelfName() const
{
	return std::string_view(selfName.data());
}

std::string_view CPythonExchange::GetPlayerTargetName() const
{
	return std::string_view(targetName.data());
}

void CPythonExchange::SetLevelToTarget(DWORD level)
{
