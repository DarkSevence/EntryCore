...
void CPythonApplication::OnKeyDown(int iIndex)
{
	...
	if (DIK_TAB == iIndex)
	{
		const auto currentPlayer = CPythonPlayer::InstancePtr();
		if (currentPlayer)
		{
			const auto nextTargetInstance = CPythonCharacterManager::Instance().GetTabNextTargetPointer(currentPlayer->NEW_GetMainActorPtr());
			if (nextTargetInstance)
			{
				const auto targetVirtualID = nextTargetInstance->GetVirtualID();
				currentPlayer->SetTarget(targetVirtualID, true);
			}
		}
	}
