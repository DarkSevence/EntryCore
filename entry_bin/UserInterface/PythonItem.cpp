...
void CPythonItem::CreateItem(uint32_t dwVirtualID, uint32_t dwVirtualNumber, uint32_t dwItemCount, float x, float y, float z, bool bDrop)
{
	//CItemManager& rkItemMgr=CItemManager::Instance();
	...
	if (dwItemCount > 1) 
	{
		itemName = std::to_string(dwItemCount) + "x - " + itemName;
	}

	CPythonTextTail& rkTextTail = CPythonTextTail::Instance();
	rkTextTail.RegisterItemTextTail(dwVirtualID, itemName.c_str(), &pGroundItemInstance->ThingInstance);
}
