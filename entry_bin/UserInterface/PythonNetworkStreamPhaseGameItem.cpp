...
bool CPythonNetworkStream::SendSafeBoxCheckinPacket(TItemPos InventoryPos, BYTE bySafeBoxPos, bool isRightClickAutoMoveEnabled)
{
	...
	kSafeboxCheckin.isRightClickAutoMoveEnabled = isRightClickAutoMoveEnabled;
...
bool CPythonNetworkStream::SendSafeBoxCheckoutPacket(BYTE bySafeBoxPos, TItemPos InventoryPos, bool isRightClickAutoMoveEnabled)
{
	...
	kSafeboxCheckout.isRightClickAutoMoveEnabled = isRightClickAutoMoveEnabled;
...
//////////////////////////////////////////////////////////////////////////
// Mall
bool CPythonNetworkStream::SendMallCheckoutPacket(BYTE byMallPos, TItemPos InventoryPos, bool isRightClickAutoMoveEnabled)
{
	...
	kMallCheckoutPacket.isRightClickAutoMoveEnabled = isRightClickAutoMoveEnabled;
...
bool CPythonNetworkStream::RecvItemGroundAddPacket()
{
	...
	if (!Recv(sizeof(TPacketGCItemGroundAdd), &packetItemGroundAdd))
	{
		return false;
	}

	__GlobalPositionToLocalPosition(packetItemGroundAdd.x, packetItemGroundAdd.y);
	CPythonItem::Instance().CreateItem(packetItemGroundAdd.dwVID, packetItemGroundAdd.dwVnum, packetItemGroundAdd.dwItemCount, packetItemGroundAdd.x, packetItemGroundAdd.y, packetItemGroundAdd.z);
	return true;
}

bool CPythonNetworkStream::RecvItemGroundDelPacket()
{
	TPacketGCItemGroundDel itemGroundDeletePacket;

	if (!Recv(sizeof(TPacketGCItemGroundDel), &itemGroundDeletePacket))
	{
		return false;
	}

	CPythonItem::Instance().DeleteItem(itemGroundDeletePacket.dwVID);
	return true;
}

bool CPythonNetworkStream::RecvItemOwnership()
{
	TPacketGCItemOwnership itemOwnershipPacket;

	if (!Recv(sizeof(TPacketGCItemOwnership), &itemOwnershipPacket))
	{
		return false;
	}

	CPythonItem::Instance().SetOwnership(itemOwnershipPacket.dwVID, itemOwnershipPacket.szName);
	return true;
}

bool CPythonNetworkStream::RecvItemUsePacket()
{
	TPacketGCItemUse itemUsePacket;

	if (!Recv(sizeof(TPacketGCItemUse), &itemUsePacket))
	{
		return false;
	}
...
{
	TPacketGCItemUpdate itemUpdateInfo;

	if (!Recv(sizeof(TPacketGCItemUpdate), &itemUpdateInfo))
	{
		return false;
	}

	auto& player = IAbstractPlayer::GetSingleton();
	player.SetItemCount(itemUpdateInfo.Cell, itemUpdateInfo.count);

	for (int32_t socketIndex = 0; socketIndex < ITEM_SOCKET_SLOT_MAX_NUM; ++socketIndex)
	{
		player.SetItemMetinSocket(itemUpdateInfo.Cell, socketIndex, itemUpdateInfo.alSockets[socketIndex]);
	}

	for (int32_t attributeIndex = 0; attributeIndex < ITEM_ATTRIBUTE_SLOT_MAX_NUM; ++attributeIndex)
	{
		auto& attribute = itemUpdateInfo.aAttr[attributeIndex];
		player.SetItemAttribute(itemUpdateInfo.Cell, attributeIndex, attribute.bType, attribute.sValue);
	}
...
bool CPythonNetworkStream::RecvSpecialEffect()
{
	...
	switch (kSpecialEffect.type)
	{
		...
		case SE_EQUIP_LOVE_PENDANT:
			...
			break;
		case SE_EFFECT_CAPE_OF_COURAGE:
