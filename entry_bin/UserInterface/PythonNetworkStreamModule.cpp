...
PyObject* netSendSafeboxCheckinPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	int iSafeBoxPos;
	bool isRightClickAutoMoveEnabled = false;
	...
	switch (PyTuple_Size(poArgs))
	{
		case 1:
		{
			...
			isRightClickAutoMoveEnabled = true;
		}
		break;
		case 2:
		{
	...
	CPythonNetworkStream& rns=CPythonNetworkStream::Instance();
	rns.SendSafeBoxCheckinPacket(InventoryPos, iSafeBoxPos, isRightClickAutoMoveEnabled);
...
PyObject* netSendSafeboxCheckoutPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rns=CPythonNetworkStream::Instance();
	rns.SendSafeBoxCheckoutPacket(iSafeBoxPos, InventoryPos, isRightClickAutoMoveEnabled);
...
PyObject* netSendMallCheckoutPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendMallCheckoutPacket(iMallPos, InventoryPos, isRightClickAutoMoveEnabled);
...
{
	const CPythonNetworkStream::CTIPMANAGER* TipManager = CPythonNetworkStream::Instance().TipManager.get();

	if (TipManager == nullptr)
	{
		return Py_BuildValue("s", "");
	}

	long lMapIndex = CPythonBackground::Instance().GetWarpMapIndex();
	...
	const std::string* TipInfo = TipManager->GetTip(lMapIndex);
	return Py_BuildValue("s", TipInfo ? TipInfo->c_str() : "");
}
