...
const  CPythonNonPlayer::TMobTable* CPythonNonPlayer::GetValidTable(uint32_t dwVnum)
{
	const TMobTable* c_pTable = GetTable(dwVnum);

	if (!c_pTable)
	{
		TraceError("CPythonNonPlayer::GetValidTable - Invalid table for VNUM %u", dwVnum);
		return nullptr;
	}
...
{
	const TMobTable* c_pTable = GetValidTable(dwVnum);
	return c_pTable ? c_pTable->bLevel : 0;
}
#endif
...
{
	const TMobTable* c_pTable = GetValidTable(dwVnum);
	return c_pTable ? ((c_pTable->dwAIFlag) & (CInstanceBase::EMobAIFlags::AIFLAG_AGGRESSIVE)) : false;
}
#endif
#endif
