...
void CPythonPlayer::SetTarget(DWORD dwVID, BOOL bForceChange)
{
	...
	if (__IsSameTargetVID(dwVID))
	{
		...
		m_dwTargetEndTime = dwCurrentTime + (bForceChange ? 500 : 1000); // Zmniejszona warto쒏 dla szybszej reakcji
		return;
	}

	if (m_dwTargetEndTime > dwCurrentTime && !bForceChange)
		return;

	m_dwTargetEndTime = dwCurrentTime + 1000; // Sta쿪 warto쒏 dla regularnej zmiany celu
