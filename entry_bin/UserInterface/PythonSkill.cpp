...
BOOL CPythonSkill::SSkillData::IsIgnoreBlockSkill()
{
...
static PyObject* skillGetSkillCoolTime(PyObject* poSelf, PyObject* poArgs)
{
	...
	{
		return Py_BuildException("skill.GetSkillCoolTime - Requires exactly 2 arguments, but received %d", PyTuple_Size(poArgs));
	}

	int32_t skillIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &skillIndex))
	{
		return Py_BuildException("skill.GetSkillCoolTime - First argument must be an integer (skill index)");
	}

	float skillLevel;
	if (!PyTuple_GetFloat(poArgs, 1, &skillLevel))
	{
		return Py_BuildException("skill.GetSkillCoolTime - Second argument must be a float (skill point)");
	}

	CPythonSkill::SSkillData* pSkillData;
	if (!CPythonSkill::Instance().GetSkillData(skillIndex, &pSkillData))
	{
		return Py_BuildException("skill.GetSkillCoolTime - Failed to find skill data for index %d", skillIndex);
	}

	uint32_t baseCooldown = pSkillData->GetSkillCoolTime(skillLevel);
	int32_t castingSpeedModifier = CPythonPlayer::Instance().GetStatus(POINT_CASTING_SPEED);

	int32_t cooldownModifier = 100 - castingSpeedModifier;
	if (cooldownModifier > 0) 
	{
		cooldownModifier = 100 + cooldownModifier;
	}
	else if (cooldownModifier < 0) 
	{
		cooldownModifier = 10000 / (100 - cooldownModifier);
	}
	else 
	{
	...
	uint32_t adjustedCooldown = max(0, static_cast<int32_t>(baseCooldown * cooldownModifier / 100));

	return Py_BuildValue("i", adjustedCooldown);
}
