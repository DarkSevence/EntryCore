...
class CPythonNetworkStream : public CNetworkStream, public CSingleton<CPythonNetworkStream>
{
	...
	public:
		...
		bool SendExchangeItemAddPacket(TItemPos ItemPos, BYTE byDisplayPos, bool isRightClickAutoMoveEnabled);
		...
		bool SendSafeBoxCheckinPacket(TItemPos InventoryPos, BYTE bySafeBoxPos, bool isRightClickAutoMoveEnabled);
		bool SendSafeBoxCheckoutPacket(BYTE bySafeBoxPos, TItemPos InventoryPos, bool isRightClickAutoMoveEnabled);
		bool SendSafeBoxItemMovePacket(BYTE bySourcePos, BYTE byTargetPos, BYTE byCount);


		// Mall
		bool SendMallCheckoutPacket(BYTE byMallPos, TItemPos InventoryPos, bool isRightClickAutoMoveEnabled);
	...
	protected:
		...
		bool RecvGuildMarkUpdatePacket();
	...
	public:
	void DownloadMark(DWORD dwGuildID);
	...
	protected:
		...
		bool HandleQuestDestroyCommand(const std::string& strConst);
	...
	class CTIPMANAGER final
	{
		...
		private:
			using TIPVEC = std::vector<std::unique_ptr<std::string>>; 

			struct STIPGROUP
			{
				STIPGROUP(std::vector<long>&& mapIndices, std::vector<uint16_t>&& tipIndices);
				~STIPGROUP() = default;

				std::vector<long> vMapIdx;
				bool IsMapExist(long MapIndex) const;

				std::vector<uint16_t> vTipIdx;
				bool IsIndexExist(uint16_t Index) const;

				std::vector<std::string*> vLocalTipVnum;

			};

			std::vector<std::unique_ptr<STIPGROUP>> vTipGroup;
			TIPVEC vTipVnum;
	};

	std::unique_ptr<CTIPMANAGER> TipManager;
