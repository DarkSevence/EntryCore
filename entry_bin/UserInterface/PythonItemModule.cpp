...
PyObject * itemGetItemType(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!pItemData)
	{
		return Py_BuildException("no selected item data er");
	}
...
PyObject * itemGetIconInstance(PyObject * poSelf, PyObject * poArgs)
{
	...
	//TraceError("Player race: %d", playerInst.GetRace());
...
PyObject* itemCreateItem(PyObject* poSelf, PyObject* poArgs)
{
	...
	float x;
	if (!PyTuple_GetFloat(poArgs, 3, &x))
		return Py_BadArgument();
	float y;
	if (!PyTuple_GetFloat(poArgs, 4, &y))
		return Py_BadArgument();
	float z;
	if (!PyTuple_GetFloat(poArgs, 5, &z))
		return Py_BadArgument();

	bool bDrop = true;
	PyTuple_GetBoolean(poArgs, 6, &bDrop);

	CPythonItem::Instance().CreateItem(iVirtualID, iVirtualNumber, iItemCount, x, y, z, bDrop);
...
void initItem()
{
	...
	PyModule_AddIntConstant(poModule, "USE_BUCKET", CItemData::USE_BUCKET);
	PyModule_AddIntConstant(poModule, "USE_POTION_CONTINUE", CItemData::USE_POTION_CONTINUE);
	...
	PyModule_AddIntConstant(poModule, "USE_RECIPE", CItemData::USE_RECIPE);
	PyModule_AddIntConstant(poModule, "USE_CHANGE_ATTRIBUTE2", CItemData::USE_CHANGE_ATTRIBUTE2);
	...
	PyModule_AddIntConstant(poModule, "ITEM_FLAG_UNUSED03", CItemData::ITEM_FLAG_UNUSED03);
	...
	PyModule_AddIntConstant(poModule, "APPLY_CURSE_PCT", CItemData::APPLY_CURSE_PCT);
	...
	PyModule_AddIntConstant(poModule, "APPLY_EXTRACT_HP_PCT", CItemData::APPLY_EXTRACT_HP_PCT);
	...
	PyModule_AddIntConstant(poModule, "APPLY_DEF_GRADE", CItemData::APPLY_DEF_GRADE);
