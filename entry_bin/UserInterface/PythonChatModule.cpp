...
{
	int32_t chatSetId;
	if (!PyTuple_GetInteger(poArgs, 0, &chatSetId))
	{
		PyErr_SetString(PyExc_ValueError, "Invalid argument");
		return nullptr;
	}

	int32_t chatModeFlags;
	bool isModeActive = CPythonChat::Instance().GetChatMode(chatSetId, &chatModeFlags);
	return Py_BuildValue("Oi", PyBool_FromLong(isModeActive), chatModeFlags);
}
