...
void CPythonPlayer::__Update_AutoAttack()
{
	...
	// ĹşČŻ°Ý ľ˛°í ´Ţ·Á°ˇ´Â µµÁßżˇ´Â ˝şĹµ
...
{
	assert(!"CPythonPlayer::SetSkillLevel - »çżëÇĎÁö ľĘ´Â ÇÔĽö");
	if (dwSlotIndex >= SKILL_MAX_NUM)
		return;
...
void CPythonPlayer::NEW_ClearSkillData(bool bAll)
{
	...
	{
		// 2004.09.30.myevan.˝şĹł°»˝Ĺ˝Ă ˝şĹł Ć÷ŔÎĆ®ľ÷[+] ąöĆ°ŔĚ ľČłŞżÍ Ăł¸®
...
void CPythonPlayer::DisableToggleActiveSkills(DWORD dwSlotIndex)
{
	auto* mainCharacterPtr = NEW_GetMainActorPtr();

	if (!mainCharacterPtr || !mainCharacterPtr->IsDead() || dwSlotIndex >= SKILL_MAX_NUM)
	{
		return;
	}

	const DWORD maxSkillIndex = SKILL_MAX_NUM - 1;

	for (; dwSlotIndex <= maxSkillIndex; ++dwSlotIndex)
	{
