...
void CPythonChat::SChatLine::SetColor(DWORD dwID, DWORD dwColor)
{
	...
	{
		assert(false && "Invalid dwID provided. Out of range.");
	}
}

void CPythonChat::SChatLine::SetColorAll(DWORD dwColor)
{
	std::fill_n(aColor, CHAT_LINE_COLOR_ARRAY_MAX_NUM, dwColor);
}
...
int CPythonChat::GetLineCount(DWORD dwID)
{
	...
	for (const auto& line : ChatLineDeque)
	{
		if (pChatSet->CheckMode(line->iType))
		{
			++iCount;
		}
	}
...
bool CPythonChat::GetChatMode(uint32_t chatSetId, int32_t* chatModeFlags)
{
	if (chatModeFlags == nullptr)
	{
		return false;
	}

	TChatSet* chatSetPtr = GetChatSetPtr(chatSetId);
	...
	constexpr uint8_t availableChatModes[] = {CHAT_TYPE_TALKING, CHAT_TYPE_INFO, CHAT_TYPE_NOTICE, CHAT_TYPE_PARTY, CHAT_TYPE_GUILD, CHAT_TYPE_SHOUT};

	int32_t accumulatedModes = 0;

	for (auto mode : availableChatModes)
	{
		if (chatSetPtr->CheckMode(mode))
		{
			accumulatedModes |= 1 << mode;
		}
	}

	*chatModeFlags = accumulatedModes;
	return accumulatedModes > 0;
}
...
{
	auto pkDefaultFont = static_cast<CGraphicText*>(DefaultFont_GetResource());
	...
	auto* pChatLine = SChatLine::New();

	if (!pChatLine)
	{
		TraceError("CPythonChat::AppendChatLog - FAILED_TO_CREATE_CHAT_LINE");
		return;
	}
	...
	{
		SChatLine::Delete(m_ChatLogLineDeque.front());
		m_ChatLogLineDeque.pop_front();
	}

	for (auto& [id, chatSet] : m_ChatSetMap)
	{
		chatSet.m_ShowingChatLogLineList.emplace_back(pChatLine);

		if (chatSet.m_ShowingChatLogLineList.size() > CHAT_LINE_MAX_NUM)
		{
			chatSet.m_ShowingChatLogLineList.pop_front();
		}
	}
}
...
{
	auto* chatSet = GetChatSetPtr(dwID);
	...
	chatSet->m_ShowingChatLogLineList.clear();

	TChatLineDeque filteredChatLines;

	for (auto* chatLine : m_ChatLogLineDeque)
	{
		if (chatSet->CheckMode(chatLine->iType))
		{
			filteredChatLines.emplace_back(chatLine);
		}
	}

	const int32_t currentLineCount = filteredChatLines.size();
	const int32_t visibleLineCount = min(currentLineCount, (chatSet->m_iHeight + chatSet->m_iStep) / chatSet->m_iStep);
	const int32_t endLineIndex = visibleLineCount + static_cast<int32_t>(static_cast<float>(currentLineCount - visibleLineCount - 1) * chatSet->m_fEndPos);

	int32_t currentHeight = 12;

	for (auto it = filteredChatLines.rbegin(); it != filteredChatLines.rend(); ++it)
	{
		auto* chatLine = *it;

		if (!chatSet->CheckMode(chatLine->iType) || currentHeight > chatSet->m_iHeight)
		{
			break;
		}

		chatSet->m_ShowingChatLogLineList.emplace_front(chatLine);
		currentHeight += chatSet->m_iStep;
	}
}
