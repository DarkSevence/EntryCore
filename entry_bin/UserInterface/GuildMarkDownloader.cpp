...
bool CGuildMarkDownloader::__LoginState_RecvMarkIndex()
{
	...
	CPythonTextTail::Instance().RefreshAllMarks();
	...
	if (kPacket.count > 0)
	{
		...
		if (CGuildMarkManager::Instance().GetMarkImageFilename(kPacket.imgIdx, imagePath))
		{
			...
			if (pResource->IsType(CGraphicImage::Type()))
			{
				...
				Tracef("CGuildMarkDownloader::__LoginState_RecvMarkBlock - Reloading image %s\n", imagePath.c_str());  // DEBUG
				pkGrpImg->Reload();  // Od쐗ie풹nie tekstury
			}
		}
	}
...
bool CGuildMarkDownloader::ConnectToDownloadAndSave(const CNetworkAddress& c_rkNetAddr, DWORD dwHandle, DWORD dwRandomKey)
{
	__OfflineState_Set();

	m_dwHandle = dwHandle;
	m_dwRandomKey = dwRandomKey;
	m_dwTodo = TODO_RECV_MARK_AND_SAVE;
	return CNetworkStream::Connect(c_rkNetAddr);
}