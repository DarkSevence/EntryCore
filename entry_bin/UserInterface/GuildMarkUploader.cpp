...
CGuildMarkUploader::CGuildMarkUploader() : m_pbySymbolBuf(nullptr)
{
...
bool CGuildMarkUploader::__Load(const char* c_szFileName, UINT* peError)
{
	...
	// Jeúli format nie jest BGRA, konwertujemy do BGRA, ale nie przekszta≥camy dalej do RGBA
	...
	// Kopiowanie danych obrazu do bufora cz≥onkowskiego
	memcpy(m_kMark.m_apxBuf, pixels, SGuildMark::WIDTH * SGuildMark::HEIGHT * 4);
...
bool CGuildMarkUploader::__SendMarkPacket()
{
	...
	// Zak≥adamy, øe istnieje funkcja, ktÛra zwraca VID g≥Ûwnego gracza
	CInstanceBase* pMainInstance = CPythonCharacterManager::Instance().GetMainInstancePtr();
	if (!pMainInstance)
	{
		// Obs≥uø b≥πd braku g≥Ûwnego gracza
		return false;
	}
	kPacketMarkUpload.vid = pMainInstance->GetVirtualID(); // Ustawiamy VID
