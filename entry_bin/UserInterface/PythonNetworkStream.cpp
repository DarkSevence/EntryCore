...
class CMainPacketHeaderMap : public CNetworkPacketHeaderMap
{
	...
	public:
		CMainPacketHeaderMap()
		{
			...
			Set(HEADER_GC_GUILD_MARK_UPDATE, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCGuildMarkUpdate), STATIC_SIZE_PACKET));
		}
};
...
{
	// żř·ˇ´Â ÇÎŔĚ ľČżĂ¶§ ĂĽĹ©ŔĚłŞ Ľ­ąö¶ű Á¤Č®Č÷ ¸ÂĂßľîľß ÇŃ´Ů.
	return false;
}
...
void CPythonNetworkStream::__DownloadAndSaveMark()
{
	CGuildMarkDownloader& rkGuildMarkDownloader = CGuildMarkDownloader::Instance();
	rkGuildMarkDownloader.ConnectToDownloadAndSave(m_kMarkAuth.m_kNetAddr, m_kMarkAuth.m_dwHandle, m_kMarkAuth.m_dwRandomKey);
}
...
bool CPythonNetworkStream::RecvDefaultPacket(int header)
{
	...
	TraceError("Ăł¸®µÇÁö ľĘŔş ĆĐĹ¶ Çě´ő %d, state %s\n", header, m_strPhase.c_str());
	ClearRecvBuffer();
	return true;
}
