...
{
	std::string_view nameView = CPythonExchange::Instance().GetPlayerSelfName();
	return Py_BuildValue("s", nameView.data());
}

PyObject* exchangeGetNameFromTarget(PyObject* poTarget, PyObject* poArgs)
{
	std::string_view nameView = CPythonExchange::Instance().GetPlayerTargetName();
	return Py_BuildValue("s", nameView.data());
}
