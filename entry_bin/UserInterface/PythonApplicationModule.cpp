...
PyObject* appRunPythonFile(PyObject* poSelf, PyObject* poArgs)
{
	...
	std::optional<bool> retOpt = CPythonLauncher::Instance().RunFile(szFileName);

	if (!retOpt.has_value()) {
		// ZwrÛÊ b≥πd lub wyjπtek, bo std::optional jest pusty
		return Py_BuildException();
	}

	bool ret = retOpt.value();
