...
void CEffectManager::Update()
{
	...
	// 2004. 3. 1. myevan. ÀÌÆåÆ® ¸ð´ÏÅÍ¸µ ÇÏ´Â ÄÚµå
	/*
...
BOOL CEffectManager::RegisterEffect(const char * c_szFileName,bool isExistDelete,bool isNeedCache)
{
	...
	if (m_kEftDataMap.end() != itor)
	{
		...
		{
			//TraceError("CEffectManager::RegisterEffect - Effect [%s] already exists", c_szFileName);
			return TRUE;
		}
	}
	...
	{
		//TraceError("CEffectManager::RegisterEffect - Failed to load effect script: %s", c_szFileName);
		CEffectData::Delete(pkEftData);
		return FALSE;
	}
...
int CEffectManager::CreateEffect(const char * c_szFileName, const D3DXVECTOR3 & c_rv3Position, const D3DXVECTOR3 & c_rv3Rotation)
{
	...
	DWORD dwID = GetCaseCRC32(c_szFileName, std::strlen(c_szFileName));

	// Dodanie logu wyœwietlaj¹cego nazwê pliku
	Tracef("CEffectManager::CreateEffect - Creating effect from file: %s (ID: %d)\n", c_szFileName, dwID);
