...
std::optional<bool> CPythonLauncher::RunFile(const char* const c_szFileName)
{
	CMappedFile file;
	const VOID* pvData;
	DWORD dwBufSize = 0;
	...
	std::vector<char> acBufData(dwBufSize); // Liskov Substitution & Interface Segregation
	std::memcpy(acBufData.data(), pvData, dwBufSize); // Open/Closed

	return RunMemoryTextFile(c_szFileName, dwBufSize, acBufData.data()); // Dependency Inversion
}
