...
bool CTextureSet::SetTexture(unsigned long ulIndex, const char * c_szFileName,float fuScale, float fvScale, float fuOffset, float fvOffset, bool bSplat, unsigned short usBegin, unsigned short usEnd, float fTerrainTexCoordBase)
{
	...
	if (!c_szFileName)
	{
		TraceError("CTextureSet::SetTexture: Nazwa pliku jest nullptr.", "B³¹d");
		return false;
	}
	...
	{
		TraceError("CTextureSet::SetTexture: Nie mo¿na ustawiæ tekstury. Zasób '%s' jest nieistniej¹cy lub nie jest obiektem typu CGraphicImage.", c_szFileName ? c_szFileName : "Nieznany");
		return false;
	}
...
bool CTextureSet::AddTexture(const char * c_szFileName, float fuScale, float fvScale, float fuOffset, float fvOffset, bool bSplat, unsigned short usBegin, unsigned short usEnd, float fTerrainTexCoordBase)
{
	if (!c_szFileName)
	{
		LogBox("CTextureSet::AddTexture: Nazwa pliku jest nullptr.", "B³¹d");
		return false;
	}

	if (GetTextureCount() >= 256)
	{
		LogBox("Osi¹gniêto maksymaln¹ liczbê tekstur.", "Limit 255");
		return false;
	}

	for (unsigned long i = 0; i < GetTextureCount(); ++i)
	{
		if (m_Textures[i].stFilename.compare(c_szFileName) == 0)
		{
			LogBox("Tekstura o podanej nazwie ju¿ istnieje.", "Powtórzenie");
			return false;
		}
	}
	...
	if (!pResource || !pResource->IsType(CGraphicImage::Type()))
	{
		TraceError("CTextureSet::SetTexture: Nie mo¿na ustawiæ tekstury. Tekstura '%s' nie istnieje", c_szFileName ? c_szFileName : "Nieznany");
		return false;
	}
	
	m_Textures.reserve(GetTextureCount() + 1);
	SetTexture(GetTextureCount(), c_szFileName, fuScale, fvScale, fuOffset, fvOffset, bSplat, usBegin, usEnd, fTerrainTexCoordBase);
