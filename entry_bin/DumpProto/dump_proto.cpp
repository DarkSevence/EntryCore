...
bool BuildMobTable()
{
	...
	if(!test_data.Load("mob_proto_test.txt",'\t'))
	{
		...
		while (test_data.Next())	//테스트 데이터 각각을 훑어나가며,새로운 것을 추가한다.
		{
			...
			fprintf(stdout, "[New]MOB #%-5d %-16s sight: %u color %u[%s]\n", mob_table->dwVnum, mob_table->szLocaleName, mob_table->wAggressiveSight, mob_table->dwMonsterColor, test_data.AsStringByIndex(54));
...
bool BuildItemTable()
{
	...
	{
		fprintf(stderr, "item_names.txt file could not be loaded\n");
		isNameFile = false;
	}
	else {
		nameData.Next();
	...
	{
		fprintf(stderr, "item_proto_test.txt file could not be loaded\n");
	}
	else {
		test_data.Next(); // skip header
		...
		while (test_data.Next()) {
			...
			{
				fprintf(stderr, "Error setting proto item table.\n");
			}
	...
	{
		fprintf(stderr, "item_proto.txt file could not be loaded\n");
		return false;
	}
...
void SaveItemProto() {
	...
	DWORD fourcc_check, dwElements_check;
	fread(&fourcc_check, sizeof(DWORD), 1, fp);
	fread(&dwElements_check, sizeof(DWORD), 1, fp);

	printf("Elements Check %u fourcc match %d\n", dwElements_check, fourcc_check == MAKEFOURCC('M', 'I', 'P', 'T'));
	fclose(fp);
}
