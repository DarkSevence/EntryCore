...
#include <algorithm>
#include <cctype>
#include <iostream> // Dodano do logowania
...
const char* cCsvAlias::operator [] (size_t index) const
{
    ...
    if (itr == m_Index2Name.end())
    {
        ...
        std::cerr << "Error: cannot find suitable conversion for index " << index << std::endl;
...
size_t cCsvAlias::operator [] (const char* name) const
{
    ...
    {
        LogToFile(NULL, "cannot find suitable conversion for ");
        std::cerr << "Error: cannot find suitable conversion for name " << name << std::endl;
        Assert(false && "cannot find suitable conversion");
        return 0;
    }
...
bool cCsvFile::Load(const char* fileName, const char seperator, const char quote)
{
    ...
    std::cout << "Finished loading CSV file: " << fileName << std::endl;
    return true;
}
...
const cCsvRow* const cCsvTable::CurRow() const
{
    if (m_CurRow < 0)
    {
        ...
        std::cerr << "Error: call Next() first!" << std::endl;
        return NULL;
    }
    else if (m_CurRow >= (int)m_File.GetRowCount())
    {
        Assert(false && "no more rows!");
        std::cerr << "Error: no more rows!" << std::endl;
        return NULL;
    }
