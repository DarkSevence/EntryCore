...
void CResourceManager::LoadStaticCache(const char* c_szFileName)
{
	...
	if (!pkRes)
	{
		TraceError("ERROR: CResourceManager::LoadStaticCache %s - FAILED to find resource", c_szFileName);
		return;
	}
	...
	Tracef("INFO: CResourceManager::LoadStaticCache %s - Loaded and cached", c_szFileName);
}
...
void CResourceManager::ProcessBackgroundLoading()
{
	...
	while (itor != m_RequestMap.end())
	{
		...
		{
			Tracef("DEBUG: Skipping %s as it is already being processed or loaded", stFileName.c_str());
		...
		itor = m_RequestMap.erase(itor);
		Tracef("INFO: Requesting background loading for %s", stFileName.c_str());
	}
	...
	while (itorRef != m_pResRefDecreaseWaitingMap.end())
	{
		...
		if (lCurrentTime - rCreatingTime > c_Reference_Decrease_Wait_Time)
		{
			...
			Tracef("DEBUG: Decreased reference count for preloaded resource after %d milliseconds", c_Reference_Decrease_Wait_Time);
		}
		else
			++itorRef;
	}
...
void CResourceManager::PushBackgroundLoadingSet(std::set<std::string> & LoadingSet)
{
	...
	while (itor != LoadingSet.end())
	{
		...
		{
			Tracef("DEBUG: %s is already loaded or being processed", itor->c_str());
			++itor;
			continue;
		}

		m_RequestMap.insert(TResourceRequestMap::value_type(dwFileCRC, itor->c_str()));
		Tracef("INFO: Added %s to background loading set", itor->c_str());
		++itor;
	}
}
