...
bool CNetworkStream::Connect(const CNetworkAddress& c_rkNetAddr, int limitSec)
{
	...
	const int32_t maxRetryCount = 5; // Maksymalna liczba prób
	int32_t retryDelay = 1000; // Domyślne opóźnienie w ms (1 sekunda)

	for (int32_t retry = 0; retry < maxRetryCount; ++retry)
	{
		if (connect(m_sock, reinterpret_cast<PSOCKADDR>(&m_addr), m_addr.GetSize()) == SOCKET_ERROR)
		{
			...
			if (error != WSAEWOULDBLOCK)
			{
				...
				{
					Tracen("Przekroczono maksymalną liczbę prób połączenia. Nie udało się połączyć.");
					Clear();
					OnConnectFailure();
					return false;
				}
				...
				retryDelay *= 2; // Podwajanie opóźnienia przy każdej próbie

				// Ponowna inicjalizacja gniazda
				Clear();
				m_sock = socket(AF_INET, SOCK_STREAM, 0);
				if (m_sock == INVALID_SOCKET)
				{
					Tracen("Nie można utworzyć nowego gniazda po błędzie połączenia.");
					Clear();
					OnConnectFailure();
					return false;
				}
		...
		{
			Tracen("Połączenie nawiązane pomyślnie.");
			break;
		}
	}

	m_connectLimitTime = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now() + std::chrono::seconds(limitSec));
	return true;
}

bool CNetworkStream::Connect(uint32_t dwAddr, int16_t port, int16_t limitSec)
{
	std::string szAddr = std::format("{}.{}.{}.{}",(dwAddr & 0xFF),(dwAddr >> 8) & 0xFF,(dwAddr >> 16) & 0xFF,(dwAddr >> 24) & 0xFF);
	return Connect(szAddr.c_str(), port, limitSec);
}

bool CNetworkStream::Connect(const char* c_szAddr, int16_t port, int16_t limitSec)
{
