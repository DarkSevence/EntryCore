...
class CNetworkStream
{
	public:
		...
		bool Connect(const char* c_szAddr, int16_t port, int16_t limitSec = 3);
		bool Connect(uint32_t dwAddr, int16_t port, int16_t limitSec = 3);
	...
	private:
		...
		char	m_szEncryptKey[TEA_KEY_LENGTH]; // Client żˇĽ­ ş¸łľ ĆĐĹ¶Ŕ» Encrypt ÇŇ¶§ »çżëÇĎ´Â Key
		char	m_szDecryptKey[TEA_KEY_LENGTH]; // Server żˇĽ­ ŔüĽŰµČ ĆĐĹ¶Ŕ» Decrypt ÇŇ¶§ »çżëÇĎ´Â Key
#endif
