...
void CStateManager::SetAnisotropicFiltering()
{
	D3DCAPS8 d3dCaps;
	...
	DWORD dwMaxAnisotropy = min(d3dCaps.MaxAnisotropy, 4UL);

	for (int i = 0; i < 8; ++i)
	{
		m_lpD3DDev->SetTextureStageState(i, D3DTSS_MAXANISOTROPY, dwMaxAnisotropy);
		m_lpD3DDev->SetTextureStageState(i, D3DTSS_MINFILTER, m_dwBestMinFilter);
		m_lpD3DDev->SetTextureStageState(i, D3DTSS_MAGFILTER, m_dwBestMagFilter);
	}

	m_lpD3DDev->SetRenderState(D3DRS_SOFTWAREVERTEXPROCESSING, FALSE);
}
