...
{
	TraceError("CGraphicMarkInstance::Load - Start loading image");

	if (GetImageFileName().empty())
	{
		TraceError("CGraphicMarkInstance::Load - Image file name is empty");
		return;
	}
	...
	if (!pResource)
	{
		TraceError("CGraphicMarkInstance::Load - [%s] resource does not exist", GetImageFileName().c_str());
		return;
	}
	...
	if (pResource->IsType(CGraphicImage::Type()))
	{
		...
		TraceError("CGraphicMarkInstance::Load - Image loaded successfully [%s]", GetImageFileName().c_str());
	}
	else
	{
		TraceError("CGraphicMarkInstance::Load - [%s] resource is not a CGraphicImage", GetImageFileName().c_str());
		return;
	}

	TraceError("CGraphicMarkInstance::Load - End loading image");
}


void CGraphicMarkInstance::ReloadImage()
{
	if (!m_roImage.IsNull())
	{
		m_roImage->Reload();
		TraceError("CGraphicMarkInstance::ReloadImage - Image reloaded [%s]", GetImageFileName().c_str());
	}
	else
	{
		TraceError("CGraphicMarkInstance::ReloadImage - No image to reload");
	}
}
