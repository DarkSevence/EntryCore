...
CSkyObjectQuad::CSkyObjectQuad() : m_Indices{0, 2, 1, 3} 
{
	std::fill_n(m_Vertex, 4, TPDTVertex{});
}

void CSkyObjectQuad::Clear(uint8_t c_rucNumVertex, float c_rfRed, float c_rfGreen, float c_rfBlue, float c_rfAlpha)
{
	c_rucNumVertex = std::clamp(c_rucNumVertex, static_cast<uint8_t>(0), static_cast<uint8_t>(3));
	m_Helper[c_rucNumVertex].Clear(c_rfRed, c_rfGreen, c_rfBlue, c_rfAlpha);
}

void CSkyObjectQuad::SetSrcColor(uint8_t c_rucNumVertex, float c_rfRed, float c_rfGreen, float c_rfBlue, float c_rfAlpha)
{
	c_rucNumVertex = std::clamp(c_rucNumVertex, static_cast<uint8_t>(0), static_cast<uint8_t>(3));
	m_Helper[c_rucNumVertex].SetSrcColor(c_rfRed, c_rfGreen, c_rfBlue, c_rfAlpha);
}

void CSkyObjectQuad::SetTransition(uint8_t c_rucNumVertex, float c_rfRed, float c_rfGreen, float c_rfBlue, float c_rfAlpha, uint32_t dwDuration)
{
	c_rucNumVertex = std::clamp(c_rucNumVertex, static_cast<uint8_t>(0), static_cast<uint8_t>(3));
