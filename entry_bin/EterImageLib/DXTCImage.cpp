...
bool CDXTCImage::LoadHeaderFromMemory(const BYTE* c_pbMap, size_t fileSize)
{
	DWORD dwMagic;
	const size_t DDS_HEADER_SIZE = sizeof(DWORD) + sizeof(DDSURFACEDESC2);

	// Check if the file is large enough to contain the DDS header
	if (fileSize < DDS_HEADER_SIZE) 
	{
		return false;
	}
	...
	// Check if there is enough data left for the surface description
	if (fileSize < sizeof(DDSURFACEDESC2))
	{
		return false;
	}
	...
	fileSize -= sizeof(DDSURFACEDESC2);
...
bool CDXTCImage::LoadFromMemory(const BYTE* c_pbMap, size_t fileSize)
{
	if (!LoadHeaderFromMemory(c_pbMap, fileSize))
	{
		return false;
	}
