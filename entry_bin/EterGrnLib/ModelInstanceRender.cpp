...
void CGrannyModelInstance::RenderMeshNodeListWithOneTexture(CGrannyMesh::EType eMeshType, CGrannyMaterial::EType eMtrlType)
{
	...
	while (pMeshNode)
	{
		...
		bool shouldUpdateSpecularInfo = false;
		float cachedSpecularPower = 0.0f;
		if (!material_data_.pImage)
		{
			cachedSpecularPower = m_kMtrlPal.GetMaterialRef(0).GetSpecularPower();
			shouldUpdateSpecularInfo = std::fabs(cachedSpecularPower - material_data_.fSpecularPower) >= std::numeric_limits<float>::epsilon();
		}
