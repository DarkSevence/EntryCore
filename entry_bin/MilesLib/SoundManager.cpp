...
{
	Tracenf("PlayMusic: Starting to play file: %s", c_szFileName);
	...
	{
		TraceError("PlayMusic: Failed to set instance for file: %s", c_szFileName);
		return;
	}
	...
	if (!pInstance)
	{
		Tracenf("PlayMusic: No instance created for file: %s", c_szFileName);
		return;
	}
	...
	Tracenf("PlayMusic: Successfully started playing file: %s", c_szFileName);
...
BOOL CSoundManager::GetMusicIndex(const char* c_szFileName, DWORD* pdwIndex)
{
	...
	// Log informuj퉏y o pocz퉡ku procesu wyszukiwania indeksu
	Tracenf("GetMusicIndex: Starting to find index for file: %s with CRC: %u", c_szFileName, dwCRC);

	for (int i = 0; i < CSoundManagerStream::MUSIC_INSTANCE_MAX_NUM; ++i)
	{
		const TMusicInstance& c_rMusicInstance = m_MusicInstances[i];

		// Log informuj퉏y o aktualnym indeksie i jego stanie
		Tracenf("GetMusicIndex: Checking index %d, with state: %d and CRC: %u", i, c_rMusicInstance.MusicState, c_rMusicInstance.dwMusicFileNameCRC);

		if (MUSIC_STATE_OFF != c_rMusicInstance.MusicState)
		{
			if (c_rMusicInstance.dwMusicFileNameCRC == dwCRC)
			{
				// Log informuj퉏y o znalezieniu dopasowania
				Tracenf("GetMusicIndex: Found matching index %d for file: %s", i, c_szFileName);
	...
	// Log informuj퉏y, 풽 nie znaleziono dopasowania
	Tracenf("GetMusicIndex: No matching index found for file: %s", c_szFileName);
