...
{
	Tracenf("SetInstance: Checking instance index: %d", dwIndex);

	if (!CheckInstanceIndex(dwIndex)) {
		Tracenf("SetInstance: Invalid instance index: %d", dwIndex);
		return false;
	}

	HSTREAM hStream = AIL_open_stream(ms_DIGDriver, filename, 0);
	if (NULL == hStream) {
		Tracenf("SetInstance: Failed to open stream for file: %s", filename);
		return false;
	}
