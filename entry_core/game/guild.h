...
class CGuild
{
	public:
		...
		void		AddComment(LPCHARACTER character, const std::string& comment);
		void		DeleteComment(LPCHARACTER ch, DWORD comment_id);
		
		void RemoveComments(const char* playerName);
		void RefreshCommentsForAllMembers();
	...
	private:
		...
		static constexpr size_t MaxCommentLength = GUILD_COMMENT_MAX_LEN;
		static constexpr std::chrono::minutes FloodProtectionInterval{1};
		std::chrono::steady_clock::time_point lastCommentTime = std::chrono::steady_clock::now() - FloodProtectionInterval;
