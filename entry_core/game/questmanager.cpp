...
namespace quest
{
	...
	bool CQuestManager::Initialize()
	{
		...
		SetEventFlag("member_rejoin_cooldown_delay", 1);
		SetEventFlag("guild_reformation_cooldown_delay", 3);
	...
	void CQuestManager::TerminateAllQuestsAndSendUpdate()
	{
		#pragma pack(push, 1)
		struct PacketScript
		{
			BYTE header{HEADER_GC_SCRIPT};
			WORD size{0};
			BYTE skin{QUEST_SKIN_NOWINDOW};
			WORD src_size{0};
		};
		#pragma pack(pop)
		...
		for (auto& [playerID, questController] : m_mapPC)
		{
			questController.CancelRunning();
			auto character = CHARACTER_MANAGER::instance().FindByPID(playerID);

			if (!character || !(character->GetDesc()))
			{
				continue;
			}

			std::string command = "[DESTROY_ALL]";
			packetScript.src_size = static_cast<WORD>(command.size());
			packetScript.size = static_cast<WORD>(packetScript.src_size + sizeof(PacketScript));

			std::vector<char> buffer;
			buffer.resize(sizeof(PacketScript) + command.size());
			std::memcpy(buffer.data(), &packetScript, sizeof(PacketScript));
			std::memcpy(buffer.data() + sizeof(PacketScript), command.data(), command.size());

			character->GetDesc()->Packet(buffer.data(), buffer.size());
		}
	}
	...
	{
		auto currentCharacterPtr = GetCurrentCharacterPtr();
		
		if (m_bNoSend || m_strScript.empty() || !currentCharacterPtr || !currentCharacterPtr->GetDesc())
		{
			m_bNoSend = false;
			ClearScript();
			return;
		...
		bool isDoneScript = m_strScript == "[DONE]";
		bool isNextScript = m_strScript == "[NEXT]";

		if (isDoneScript || isNextScript)
		{
			if (isDoneScript && m_pCurrentPC && !m_pCurrentPC->GetAndResetDoneFlag() && m_iCurrentSkin == QUEST_SKIN_NORMAL && !IsError())
			{
				ClearScript();
				return;
			}
		...
		TEMP_BUFFER buf;
		packet_script packet{};
		packet.header = HEADER_GC_SCRIPT;
		packet.skin = m_iCurrentSkin;
		packet.src_size = m_strScript.size();
		packet.size = packet.src_size + sizeof(packet_script);

		buf.write(&packet, sizeof(packet_script)); 
		buf.write(m_strScript.c_str(), m_strScript.size());

		auto characterDesc = currentCharacterPtr->GetDesc();
		characterDesc->Packet(buf.read_peek(), buf.size());

		if (test_server) 
		{
			const char* charName = currentCharacterPtr->GetName();
			sys_log(0, "[Quest Script] Sending to %s: Script '%s', Size: %d bytes", charName, m_strScript.c_str(), buf.size());
		}
	...
	void CQuestManager::Reload()
	{
		if (!InitializeLua())
		{
			sys_err("[CQuestManager::Reload] Failed to initialize Lua during quest reload.");
			return;
		}

		TerminateAllQuestsAndSendUpdate();
		...
		for (auto& [key, event] : m_mapServerTimer) 
		{
			event_cancel(&event);
		}
		
		m_mapServerTimer.clear();
		...
		if (!Initialize())
		{
			sys_err("[CQuestManager::Reload] Failed to execute main initialization during quest reload.");
			return;
		}

		for (const auto& dwVnum : m_registeredNPCVnum) 
		{
			std::string vnumStr = std::to_string(dwVnum);
			m_mapNPC[dwVnum].Set(dwVnum, vnumStr.c_str());
		}
	}
