...
struct FCountMonster
{
	...
	void operator()(LPENTITY entityPtr)
	{
		if (entityPtr->IsType(ENTITY_CHARACTER))
		{
            LPCHARACTER characterPtr = static_cast<LPCHARACTER>(entityPtr);
            if (characterPtr->IsMonster() || characterPtr->IsStone())
			{
                ++monsterCount;
			}
		}
