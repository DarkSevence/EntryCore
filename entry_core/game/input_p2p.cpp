...
struct FuncShout
{
	...
	{
		LPCHARACTER character = d->GetCharacter();
		...
		if (!isGlobalChatEnabled && character->GetGMLevel() == GM_PLAYER && d->GetEmpire() != m_bEmpire)
		{
			return;
		}
		
		character->ChatPacket(CHAT_TYPE_SHOUT, "%s", m_str);
	}
};
