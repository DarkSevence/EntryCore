...
static bool FN_should_check_exp(LPCHARACTER character)
{
	return character->GetLevel() < gPlayerMaxLevel;
}
...
bool CHARACTER::LearnSkillByBook(DWORD dwSkillVnum, BYTE bProb)
{
	...
	if (!IsLearnableSkill(dwSkillVnum))
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;276]");
		return false;
	}
    ...
    if (FN_should_check_exp(this))
	{
		...
		if (GetExp() < need_exp)
		{
			ChatPacket(CHAT_TYPE_INFO, "[LS;277]");
			return false;
		}
	}
	...
	if (pkSk->dwType != 0)
	{
		...
		{
			if (GetSkillMasterType(dwSkillVnum) > SKILL_MASTER)
				ChatPacket(CHAT_TYPE_INFO, "[LS;278]");
			else
				ChatPacket(CHAT_TYPE_INFO, "[LS;279]");
			return false;
		}
	}
	...
	if (get_global_time() < GetSkillNextReadTime(dwSkillVnum))
	{
		if (!(test_server && quest::CQuestManager::instance().GetEventFlag("no_read_delay")))
		{
			if (FindAffect(AFFECT_SKILL_NO_BOOK_DELAY))
			{
				...
				ChatPacket(CHAT_TYPE_INFO, "[LS;280]");
			}
			else
			{
			...
			if (pPC)
			{
				...
				if (number(1, 100) > percent)
				{
					...
					else
					{
						...
						{
							case 1:
								ChatPacket(CHAT_TYPE_INFO, "[LS;282]");
								break;

							case 2:
								ChatPacket(CHAT_TYPE_INFO, "[LS;283]");
								break;
						...
						ChatPacket(CHAT_TYPE_INFO, "[LS;285;%d]", need_bookcount - read_count);
						return true;
					}
				}
			}
	...
	if (bLastLevel != GetSkillLevel(dwSkillVnum))
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;286]");
		ChatPacket(CHAT_TYPE_INFO, "[LS;287]");
		ChatPacket(CHAT_TYPE_INFO, "[LS;288]");
		LogManager::instance().CharLog(this, dwSkillVnum, "READ_SUCCESS", "");
	}
	else
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;289]");
		ChatPacket(CHAT_TYPE_INFO, "[LS;290]");
...
struct FuncSplashDamage
{
	...
	void operator () (LPENTITY ent)
	{
		...
		{
			if (m_pkWeapon && m_pkWeapon->GetSubType() == WEAPON_BELL && IS_SET(m_pkSk->dwFlag, SKILL_FLAG_KNOCKBACK))
			{
				if (!pkChrVictim->IsStone())
				{
					constexpr float knockbackDistance = 300.0f;
					float knockbackDirectionX, knockbackDirectionY;
					auto angleDegreesBetweenCharacters = GetDegreeFromPositionXY(m_pkChr->GetX(), m_pkChr->GetY(), pkChrVictim->GetX(), pkChrVictim->GetY());

					if (m_pkSk->dwVnum == SKILL_HORSE_WILDATTACK)
					{
						angleDegreesBetweenCharacters -= m_pkChr->GetRotation();
						angleDegreesBetweenCharacters = std::fmod(angleDegreesBetweenCharacters + std::numbers::pi, 360.0f);

						
						if (angleDegreesBetweenCharacters > 180.0f)
						{
							angleDegreesBetweenCharacters -= 360.0f;
						}
						
						angleDegreesBetweenCharacters += (angleDegreesBetweenCharacters > 0) ? 90.0f : -90.0f;
					}

					GetDeltaByDegree(angleDegreesBetweenCharacters, knockbackDistance, &knockbackDirectionX, &knockbackDirectionY);

					auto targetX = static_cast<long>(pkChrVictim->GetX() + knockbackDirectionX);
					auto targetY = static_cast<long>(pkChrVictim->GetY() + knockbackDirectionY);
					
					if (test_server)
					{
						m_pkChr->ChatPacket(CHAT_TYPE_INFO, "[LS;294;%s;%s;%.1f;%.1f;%.2f'%.2f]", m_pkChr->GetName(), pkChrVictim->GetName(), knockbackDistance, knockbackDirectionX, knockbackDirectionY);
					}

					pkChrVictim->Sync(targetX, targetY);
					pkChrVictim->Goto(targetX, targetY);
			...
			{
				#if defined(ENABLE_LEVEL_RESTRICTION_FOR_REMOVE_GOOD_AFFECT)
				int casterLevel = m_pkChr->GetLevel(); 
				int victimLevel = pkChrVictim->GetLevel(); 

				if (abs(casterLevel - victimLevel) >= SKILL_REMOVE_AFFECT_MAX_LEVEL_DIFF)
				{
					m_pkChr->ChatPacket(CHAT_TYPE_INFO, "[LS;297]");
					return;
				}
				#endif
				{
			...
			if (IS_SET(m_pkSk->dwFlag, SKILL_FLAG_CRUSH | SKILL_FLAG_CRUSH_LONG) && !IS_SET(pkChrVictim->GetAIFlag(), AIFLAG_NOMOVE) && m_pkChr->GetMobRank() < MOB_RANK_BOSS)
			{
...
int32_t CHARACTER::ComputeSkillParty(uint32_t dwVnum, LPCHARACTER pkVictim, uint8_t bSkillLevel)
{
  ...
  * function returns true, indicating that the buff should be returned to the caster.
  *
  * @param dwVnum The unique identifier (VNUM) of the buff/skill to be applied.
  * @param pkVictim Pointer to the character receiving the buff.
  * @param pkCaster Pointer to the character casting the buff.
  * @return true if the buff should be returned to the caster, otherwise false.
  * @ SHAREGON.PL ~ by Sevence
  */

#if defined(BLOCK_BUFF_FOR_GM)
bool CHARACTER::BlockBuffForGM(uint32_t dwVnum, LPCHARACTER pkVictim, LPCHARACTER pkCaster)
{
	std::array<uint32_t, 6> iArrayIndexSkill = {94, 95, 96, 109, 110, 111};
	bool shouldReturnBuff = false;
...
bool CHARACTER::UseSkill(DWORD dwVnum, LPCHARACTER pkVictim, bool bUseGrandMaster)
{
...
#if defined(BLOCK_BUFF_FOR_GM)
    LPCHARACTER pkCaster = this;

    if (pkVictim)
    {
        if (BlockBuffForGM(dwVnum, pkVictim, pkCaster))
        {
            pkVictim = pkCaster;
        }
    }
	...
	else
	{
		...
		if (test_server)
			ChatPacket(CHAT_TYPE_INFO, "[LS;293;%s;%d]", pkSk->szName, iNeededSP);
	...
	if (false == m_bDisableCooltime)
	{
		if (false ==
				m_SkillUseInfo[dwVnum].UseSkill(
					bUseGrandMaster,
				   	(0 != pkVictim && SKILL_HORSE_WILDATTACK != dwVnum) ? pkVictim->GetVID() : 0,
...
void CHARACTER::AddPerfectSkillLevel() 
{
    const int PERFECT_LEVEL = 40;
    int job = GetJob();
    ...
    const std::vector<DWORD> skills[JOB_MAX_NUM][SKILL_GROUP_MAX_NUM] = {
        ...
        {{61, 62, 63, 64, 65, 66}, {76, 77, 78, 79, 80, 81}}, // Szaman (Smok, Healer)
        {{91, 92, 93, 94, 95, 96}, {106, 107, 108, 109, 110, 111}} // Sura (BM, WP)
    };

    if (job < JOB_MAX_NUM && group <= SKILL_GROUP_MAX_NUM && group > 0) {
        for (DWORD skillVnum : skills[job][group - 1]) {
            if (CanUseSkill(skillVnum)) {
                SetSkillLevel(skillVnum, PERFECT_LEVEL);
				SkillLevelPacket();
				ComputePoints();
            }
        }
