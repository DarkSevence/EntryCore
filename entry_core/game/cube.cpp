...
{
	for (const auto& cube_data : s_cube_proto)
	{
		if (std::find(cube_data->npc_vnum.begin(), cube_data->npc_vnum.end(), vnum) != cube_data->npc_vnum.end())
		{
			return true;
		}
	}
...
void Cube_request_result_list(LPCHARACTER ch)
{
	...
	if (resultText.empty())
	{
		...
		const auto& resultList = cube_info_map[npcVNUM];
		for (const auto& materialInfo : resultList)
		{
			resultText += std::to_string(materialInfo.reward.vnum) + "," + std::to_string(materialInfo.reward.count) + "/";
		}
...
void Cube_request_material_info(LPCHARACTER ch, int32_t requestStartIndex, int32_t requestCount)
{
	...
	if (!bCatchInfo)
	{
		sys_err("[CubeInfo] Can't find matched material info (NPC: %u, index: %zu, request count: %zu)", npcVNUM, requestStartIndex, requestCount);
		return;
	}

	if (!materialInfoText.empty())
	{
		materialInfoText.pop_back();
	}
