...
bool CSkillManager::Initialize(TSkillTable * pTab, int iSize)
{
	...
	for (int i = 0; i < iSize; ++i, ++t)
	{
		...
		// Normalize affect flags and reset AFF_YMIR if set
        TAffectFlag affectFlag(pkProto->dwAffectFlag, pkProto->dwAffectFlag2);
        affectFlag.CheckAndResetAffectFlag2();
        pkProto->dwAffectFlag2 = affectFlag.bits[1];
