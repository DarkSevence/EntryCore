...
CHARACTER::CHARACTER()
{
    SetStateFunctions(m_stateIdle, &CHARACTER::StateIdle);
    SetStateFunctions(m_stateMove, &CHARACTER::StateMove);
    SetStateFunctions(m_stateBattle, &CHARACTER::StateBattle);

    Initialize();
}

void CHARACTER::SetStateFunctions(auto& state, auto stateFunction)
{
    state.Set(this, &CHARACTER::BeginStateEmpty, stateFunction, &CHARACTER::EndStateEmpty);
}
...
void CHARACTER::EncodeInsertPacket(LPENTITY entity)
{
	...
	pack.playerActiveAffects[0] = m_afAffectFlag.bits[0];
	pack.playerActiveAffects[1] = m_afAffectFlag.bits[1];
...
void CHARACTER::Disconnect(const char * c_pszReason)
{
	...
	{
		auto playtime = GetRealPoint(POINT_PLAYTIME);
		auto elapsed_time = (get_dword_time() - m_dwPlayStartTime) / 60000;
		auto final_value = (playtime + elapsed_time);

		packet_point_change pack
		{
			.header = HEADER_GC_CHARACTER_POINT_CHANGE,
			.dwVID = m_vid,
			.type = POINT_PLAYTIME,
			.value = final_value,
			.amount = 0
		...
		desc->Packet(&pack, sizeof(packet_point_change));
		desc->BindCharacter(nullptr);
	}
...
{
	if (!m_pkMobData)
		return MOB_RANK_KNIGHT;	// PlayerCharacter�� ��� KNIGHT��
		...
		case POINT_EXP:
			{
				...
				else
				{
					...
					if (test_server)
						ChatPacket(CHAT_TYPE_INFO, "[LS;36;%d]", amount);
		...
		case POINT_LEVEL_STEP:
			if (amount > 0)
			{
				...
				switch (val)
				{
					...
					case 3:
					{
						if (GetLevel() < maxLevelForStatPoints) PointChange(POINT_STAT, 1);
						break;
					}
...
//					AutoGiveItem(27003, 2);
		...
		case POINT_GOLD:
		{
			...
			const int64_t goldChange = static_cast<int64_t>(amount);
			const int64_t newGoldTotal = currentGold + goldChange;
			
			if (newGoldTotal < 0) 
			{
				sys_err("[GOLD_UPDATE_NEGATIVE] CurrentGold: %lld, GoldChange: %lld, PlayerID: %u, PlayerName: %s", currentGold, goldChange, GetPlayerID(), GetName());
				LogManager::instance().CharLog(this, newGoldTotal, "GOLD_UPDATE_NEGATIVE", "");
				return;
			}

			if (newGoldTotal > static_cast<int64_t>(GOLD_MAX)) 
			{
				sys_err("[GOLD_UPDATE_OVERFLOW] CurrentGold: %lld, GoldChange: %lld, PlayerID: %u, PlayerName: %s", currentGold, goldChange, GetPlayerID(), GetName());
				LogManager::instance().CharLog(this, newGoldTotal, "GOLD_UPDATE_OVERFLOW", "");
				return;
			}

			SetGold(newGoldTotal);
			val = GetGold();
		}
		break;
...
void CHARACTER::ClearMiningEvent()
{
	currentMiningEventPtr = nullptr;
}
...
void CHARACTER::CancelMining()
{
	...
	event_cancel(&currentMiningEventPtr);
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT("xxxx"));
}
...
void CHARACTER::StartMining(LPCHARACTER chLoad)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;37;]");
		return;
	}

	uint8_t count = static_cast<uint8_t>(number(5, 15));

	TPacketGCDigMotion digMotionPacket = 
	{
		.header = HEADER_GC_DIG_MOTION,
		.vid = GetVID(),
		.target_vid = chLoad->GetVID(),
		.count = count
	};

	PacketAround(&digMotionPacket, sizeof(digMotionPacket));
	currentMiningEventPtr = mining::CreateMiningEvent(this, chLoad, count);
}
...
void CHARACTER::fishing()
{
	...
	// ���� �Ӽ����� ���ø� �õ��Ѵ�?
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, "[LS;38;]");
			return;
		}
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;39;]");
		return;
	}

	if (0 == rod->GetSocket(2))
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;40;]");
		return;
	}
...
void CHARACTER::fishing_take()
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;41;]");
	}
...
void CHARACTER::RemovePart(BYTE bPartPos)
{
    assert(bPartPos < PART_MAX_NUM);

    // Ustawienie części postaci na jej oryginalną wartość
    switch (bPartPos)
    {
        case PART_MAIN:
            if (IsPC()) // Dla postaci gracza
                SetPart(bPartPos, m_pointsInstant.bBasePart);
            else // Dla NPC
                SetPart(bPartPos, GetOriginalPart(PART_MAIN));
            break;

        case PART_HAIR:
            SetPart(bPartPos, GetOriginalPart(PART_HAIR));
            break;

        // Dodaj tutaj obsługę innych części, jeśli jest to wymagane
        default:
            // Opcjonalnie: Można dodać logikę domyślnego zachowania dla innych części
            break;
    }
}
...
bool CHARACTER::RequestToParty(LPCHARACTER leader)
{
	...
	if (!leader)
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;42;]");
		return false;
	}
	...
	switch (errcode)
	{
		...
		case PERR_SERVER:
			ChatPacket(CHAT_TYPE_INFO, "[LS;43;]");
			return false;

		case PERR_DIFFEMPIRE:
			ChatPacket(CHAT_TYPE_INFO, "[LS;44;]");
			return false;

		case PERR_DUNGEON:
			ChatPacket(CHAT_TYPE_INFO, "[LS;45;]");
			return false;

		case PERR_OBSERVER:
			ChatPacket(CHAT_TYPE_INFO, "[LS;46;]");
			return false;

		case PERR_LVBOUNDARY:
			ChatPacket(CHAT_TYPE_INFO, "[LS;47;]");
			return false;

		case PERR_LOWLEVEL:
			ChatPacket(CHAT_TYPE_INFO, "[LS;48;]");
			return false;

		case PERR_HILEVEL:
			ChatPacket(CHAT_TYPE_INFO, "[LS;49;]");
			return false;
		...
		case PERR_PARTYISFULL:
			ChatPacket(CHAT_TYPE_INFO, "[LS;50;]");
			return false;
	...
	ChatPacket(CHAT_TYPE_INFO, "[LS;51;%s]", leader->GetName());
	return true;
}
...
void CHARACTER::AcceptToParty(LPCHARACTER member)
{
	...
	if (!GetParty())
		member->ChatPacket(CHAT_TYPE_INFO, "[LS;52;]");
	else 
	{
		...
		switch (errcode) 
		{
			...
			case PERR_SERVER:		member->ChatPacket(CHAT_TYPE_INFO, "[LS;43;]"); break;
			case PERR_DUNGEON:		member->ChatPacket(CHAT_TYPE_INFO, "[LS;45;]"); break;
			case PERR_OBSERVER: 	member->ChatPacket(CHAT_TYPE_INFO, "[LS;46;]"); break;
			case PERR_LVBOUNDARY:	member->ChatPacket(CHAT_TYPE_INFO, "[LS;47;]"); break;
			case PERR_LOWLEVEL: 	member->ChatPacket(CHAT_TYPE_INFO, "[LS;48;]"); break;
			case PERR_HILEVEL: 		member->ChatPacket(CHAT_TYPE_INFO, "[LS;49;]"); break;
			case PERR_ALREADYJOIN: 	break;
			case PERR_PARTYISFULL: {
									   ChatPacket(CHAT_TYPE_INFO, "[LS;50;]");
									   member->ChatPacket(CHAT_TYPE_INFO, "[LS;50;]");
									   break;
								   }
...
void CHARACTER::PartyInvite(LPCHARACTER pchInvitee)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;53;]");
		return;
	}
	else if (pchInvitee->IsBlockMode(BLOCK_PARTY_INVITE))
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;53;%s]", pchInvitee->GetName());
		return;
	}
	...
	switch (errcode)
	{
		...
		case PERR_ALREADYJOIN: 	
			ChatPacket(CHAT_TYPE_INFO, "[LS;55;]");
			return;
...
void CHARACTER::PartyInviteAccept(LPCHARACTER pchInvitee)
{
	...
	switch (errcode)
	{
		...
		case PERR_SERVER:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;43;]");
			return;

		case PERR_DUNGEON:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;45;]");
			return;

		case PERR_OBSERVER:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;46;]");
			return;

		case PERR_LVBOUNDARY:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;47;]");
			return;

		case PERR_LOWLEVEL:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;48;]");
			return;

		case PERR_HILEVEL:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;49]");
			return;

		case PERR_ALREADYJOIN:
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;55]");
			return;

		case PERR_PARTYISFULL:
			ChatPacket(CHAT_TYPE_INFO, "[LS;50]");
			pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;50]");
			return;
...
void CHARACTER::PartyInviteDeny(DWORD dwPID)
{
	...
	if (pchInvitee)
		ChatPacket(CHAT_TYPE_INFO, "[LS;54;%s]", pchInvitee->GetName());
}

void CHARACTER::PartyJoin(LPCHARACTER pLeader)
{
	pLeader->ChatPacket(CHAT_TYPE_INFO, "[LS;56;%s]", GetName());
	ChatPacket(CHAT_TYPE_INFO, "[LS;57;%s]", pLeader->GetName());
...
void CHARACTER::OnClick(LPCHARACTER pkChrCauser)
{
...

	{
		...
		{
			sys_err("OnClick Fail (%s->%s) - PlayerCharacter is exchanging", pkChrCauser->GetName(), GetName());
			return;
		}
	}
	...
	if (IsPC())
	{
		if (!CTargetManager::instance().GetTargetInfo(pkChrCauser->GetPlayerID(), TARGET_TYPE_VID, GetVID()))
		{
			...
			if (test_server)
				sys_err("%s.OnClickFailure(%s) - target is PlayerCharacter", pkChrCauser->GetName(), GetName());
...
void CHARACTER::BroadcastTargetPacket()
{
	...
	p.header = HEADER_GC_TARGET;
	p.dwVID = GetVID();
	p.bHPPercent = IsPC() ? 0 : (GetMaxHP() <= 0 ? 0 : MINMAX(0, (GetHP() * 100) / GetMaxHP(), 100));
...
bool CHARACTER::Follow(LPCHARACTER pkChr, float fMinDistance)
{
	if (IsPC())
	{
		sys_err("CHARACTER::Follow : PlayerCharacter cannot use this method", GetName());
		return false;
	}
	...
	if (IS_SET(m_pointsInstant.dwAIFlag, AIFLAG_NOMOVE))
	{
		if (pkChr->IsPC()) // �Ѿư��� ��밡 PlayerCharacter�� ��
		{
...
void CHARACTER::ReqSafeboxLoad(const char* pszPassword)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;58;]");
		return;
	}
	else if (m_pkSafebox)
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;59;]");
		return;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;60;]");
		return;
	}
...
bool CHARACTER::BuildUpdatePartyPacket(TPacketGCPartyUpdate & out)
{
	...
	if (l && DISTANCE_APPROX(GetX() - l->GetX(), GetY() - l->GetY()) < PARTY_DEFAULT_RANGE)
	{
		...
		sys_err("PARTY UPDATE: Affects set for player %s: [%d, %d, %d, %d, %d, %d, %d]", 
		        GetName(), 
		        out.affects[0], out.affects[1], out.affects[2], 
		        out.affects[3], out.affects[4], out.affects[5], out.affects[6]);			
...
void CHARACTER::ResetPoint(int iLv)
{
	...
	PointChange(POINT_STAT, (MINMAX(1, iLv, maxLevelForStatPoints) * 3) + GetPoint(POINT_LEVEL_STEP) - GetPoint(POINT_STAT));
...
int32_t CHARACTER::GetQuestFlag(const std::string& questFlagName) const
{
	if (!IsPC())
	{
		sys_err("Attempted to retrieve quest flag %s from a non-player character.", questFlagName.c_str());
		return 0;
	}
	
	uint32_t playerId = GetPlayerID();
	
	quest::CQuestManager& questManager = quest::CQuestManager::Instance();
	quest::PC* playerQuestContext = questManager.GetPC(GetPlayerID());
	
	if (!playerQuestContext )
	{
		sys_err("Failed to access quest flag %s for player with pid %u", questFlagName.c_str(), playerId);
		return 0;
	}
    
	return playerQuestContext->GetFlag(questFlagName);
}

void CHARACTER::SetQuestFlag(const std::string& questFlagName, int32_t flagValue)
{
	quest::CQuestManager& questManager = quest::CQuestManager::Instance();
	quest::PC* playerQuestContext = questManager.GetPC(GetPlayerID());
	...
	if (!playerQuestContext )
	{
		sys_err("Failed to access quest flag %s for player with pid %u", questFlagName.c_str(), pid);
		return;
	}

	playerQuestContext->SetFlag(questFlagName, flagValue);
}
...
bool CHARACTER::WarpToPID(DWORD dwPID)
{
	...
	if ((victim = (CHARACTER_MANAGER::instance().FindByPID(dwPID))))
	{
		...
		if (IS_SUMMONABLE_ZONE(mapIdx))
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, "[LS;62;]");
				return false;
			}
		}
		else
	...
	else
	{
		...
		if (!pcci)
		{
			ChatPacket(CHAT_TYPE_INFO, "[LS;63;]");
			return false;
		}

		if (pcci->bChannel != g_bChannel)
		{
			ChatPacket(CHAT_TYPE_INFO, "[LS;64;%d;%d]", pcci->bChannel, g_bChannel);
			return false;
		}
...
bool CHARACTER::IsHack(bool bSendMsg, bool bCheckShopOwner, int limittime)
{
	...
	{
		if (bSendMsg)
			ChatPacket(CHAT_TYPE_INFO, "[LS;65;%d]", limittime);
	...
	if (bCheckShopOwner)
	{
		...
		{
			if (bSendMsg)
				ChatPacket(CHAT_TYPE_INFO, "[LS;68;]");
	...
	{
		if (bSendMsg)
			ChatPacket(CHAT_TYPE_INFO, "[LS;67;%d]", limittime );
		return true;
	}
	//END_PREVENT_PORTAL_AFTER_EXCHANGE
	...
	{
		if (bSendMsg)
			ChatPacket(CHAT_TYPE_INFO, "[LS;69;%d]", limittime);
		return true;
	}
	...
	{
		if (bSendMsg)
			ChatPacket(CHAT_TYPE_INFO, "[LS;70;%d]", limittime);
		return true; 
	}
...
void CHARACTER::SetChatBlockTime(int seconds)
{
	m_iChatBlockTime = get_global_time() + seconds;
}

bool CHARACTER::IsChatBlocked() const
{
	return get_global_time() < m_iChatBlockTime;
}

int CHARACTER::GetRemainingBlockTime() const
{
	int remainingTime = m_iChatBlockTime - get_global_time();
