...
class CHARACTER : public CEntity, public CFSM, public CHorseRider
{
	...
	private:
		void SetStateFunctions(auto& state, auto stateFunction);
...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
		...
		bool IsMining() const 
		{ 
			return currentMiningEventPtr ? true : false; 
		}
		...
		void SetChatBlockTime(int seconds);
		bool IsChatBlocked() const;
		int GetRemainingBlockTime() const;

	private:
		int m_iChatBlockTime;
	...
	public:
		...
		void ClearAffect(bool shouldSaveAffects = false);
		void ComputeAffect(CAffect* pkAff, bool bAdd);

		bool AddAffect(uint32_t type, uint8_t applyOn, int32_t applyValue, uint32_t flag, int32_t duration, int32_t spCost, bool bOverride, bool IsCube = false);
		void RefreshAffect();
		bool CheckSkillAffect(uint32_t type, int32_t applyValue);
		bool RemoveAffect(uint32_t type);
		bool IsAffectFlag(uint32_t affect) const;

		bool UpdateAffect();
		int ProcessAffect();

		void LoadAffect(uint32_t affectCount, TPacketAffectElement* affectElements);
		void SaveAffect();
		...
		bool RemoveFirstActiveAffect(uint32_t type);
		void RemoveSkillAffects();
		void RemoveBadAffect();

		CAffect* FindAffect(uint32_t type, uint8_t apply = APPLY_NONE) const;
		...
		bool RemoveAffect(CAffect* affect, bool updateNow = true);
	...
	public:
		...
		static const size_t NoCellIgnored = std::numeric_limits<size_t>::max();
		int CountSpecifyItem(DWORD vnum, size_t ignoreCellIndex = NoCellIgnored) const;
		void RemoveSpecifyItem(DWORD vnum, DWORD count = 1, size_t ignoreCellIndex = NoCellIgnored);
	...
	public:
		...
		void OpenMyShop(const char* c_pszSign, TShopItemTable* itemArray, BYTE bItemCount);
	...
	public:
		...
		int32_t CalculateMagicDamage(int32_t initialDamage, int32_t magicAttBonusPer, int32_t meleeMagicAttBonusPer);
		int32_t CalculatePenetrationPercent(int32_t initialPenetratePct, bool isPC, int32_t marriageBonus, float skillResistPenetratePower, int32_t resistPenetrate);
		std::pair<int32_t, bool> CalculatePenetrationDamage(int32_t iPenetratePct, int32_t dam, bool test_server, int32_t pointDefGrade, int32_t pointDefBonus);
		...
		void 				ForgetMyAttacker(bool shouldRevive = true);
...
//		AttackLog			m_kAttackLog;
	...
	public:
		...
		bool BlockBuffForGM(uint32_t dwVnum, LPCHARACTER pkVictim, LPCHARACTER pkCaster);
#endif
	...
	public:
		...
		int32_t GetQuestFlag(const std::string& questFlagName) const;
		void SetQuestFlag(const std::string& questFlagName, int32_t flagValue);
