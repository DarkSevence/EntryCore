...
namespace marriage
{
	...
	void TMarriage::Save()
	{
		sys_log(0, "TMarriage::Save() - RequestUpdate.needsSave=%d", needsSave);
		if (needsSave)
		{
	...
	void TMarriage::Update(DWORD additionalLovePoints)
	{
		if (!IsOnline() || additionalLovePoints == 0 || !is_married)
		{
			return;
		}
		...
		uint64_t totalLovePoints = static_cast<uint64_t>(love_point) + additionalLovePoints;
		love_point = std::clamp(totalLovePoints, uint64_t(0), static_cast<uint64_t>(2000000000));

		if (test_server)
		{
			for (auto pid : {m_pid1, m_pid2})
			{
				if (auto character = CHARACTER_MANAGER::instance().FindByPID(pid); character)
				{
					std::string message = fmt::format("lovepoint bykill {:.3g} total {}", static_cast<double>(love_point) / 1000000.0, GetMarriagePoint());					
					character->ChatPacket(CHAT_TYPE_PARTY, message.c_str());
				}
			}
		}
	}
