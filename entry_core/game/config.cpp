...
bool isGlobalChatEnabled = false;
int32_t maxLevelForStatPoints = 90;
...
bool GetIPInfo()
{
	...
	{
#ifdef AUTODETECT_INTERNAL_IP_ADDRESS
	...
	*log_db = '\0';
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("global_chat")
		{
			int32_t flag = 0;
			str_to_number(flag, value_string);
			isGlobalChatEnabled = !!flag;
			fprintf(stdout, "GLOBAL_CHAT: %s\n", (isGlobalChatEnabled)?"ENABLED":"DISABLED");
			continue;
		}		

		TOKEN("MAX_LEVEL_FOR_STAT_POINTS")
		{
		    str_to_number(maxLevelForStatPoints, value_string);
		    continue;
		}
	}
