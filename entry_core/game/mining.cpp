...
namespace mining
{
	...
	void CHEAT_MAX_PICK(LPCHARACTER ch, LPITEM item)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;714;%d]", Pick_GetCurExp(pick));
	}
	...
	void PracticePick(LPCHARACTER ch, LPITEM item)
	{
		...
		if (Pick_IsPracticeSuccess(pick))
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "[LS;715]");
				ch->ChatPacket(CHAT_TYPE_INFO, "[LS;716]");
			}
			else
			{
				Pick_IncCurExp(pick);	

				ch->ChatPacket(CHAT_TYPE_INFO, "[LS;717;%d;%d]", Pick_GetCurExp(pick), Pick_GetMaxExp(pick));
	...
	EVENTFUNC(mining_event)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "[LS;718]");
			return 0;
		}
		// END_OF_REFINE_PICK
		...
		if (!load)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "[LS;719]");
			return 0;
		}
		...
		if (number(1, 100) <= iPct)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "[LS;720]");
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "[LS;721]");
		}
