...
bool CPartyManager::SetParty(LPCHARACTER ch)	// PC?? ?????? ???!!
{
...
LPPARTY CPartyManager::CreateParty(LPCHARACTER pLeader)
{
	...
	if (pLeader->IsPC())
	{
		...
		m_set_pkPCParty.insert(pParty);
		pParty->SetRole(pLeader->GetPlayerID(), PARTY_ROLE_LEADER, true);
	}
	else
	{
	...
	while (it != m_memberMap.end())
	{
		...
		if (rMember.pCharacter)
		{
			if (rMember.pCharacter->GetDesc())
			{
				...
				rMember.pCharacter->ChatPacket(CHAT_TYPE_INFO, "[LS;723]");
			}
			else
			{
				// NPC?? ??? ???? ?©£? ?? ???? ???? ??? ?? ??????? ??? ?????? ????????.
...
void CParty::Join(DWORD dwPID)
{
	...
	if (m_bPCParty)
	{
		...
		p.dwPID = dwPID;
		p.bState = PARTY_ROLE_NORMAL; // #0000790: [M2EU] CZ ????? ????: ???? ???! 
...
void CParty::Unlink(LPCHARACTER pkChr)
{
	...
	if (pkChr->IsPC())
	{
		...
		//SendPartyUnlinkAllToOne(pkChr); // ????? ?????? ???? Unlink ????? ???? ??? ????.
...
void CParty::SummonToLeader(DWORD pid)
{
	...
	{
		l->ChatPacket(CHAT_TYPE_INFO, "[LS;724]");
		return;
	}
	...
	if (n == 0)
		l->ChatPacket(CHAT_TYPE_INFO, "[LS;725]");
	else
	{
		int i = number(0, n - 1);
...
void CParty::ComputeRolePoint(LPCHARACTER ch, BYTE bRole, bool bAdd)
{
	...
	float k = static_cast<float>(ch->GetSkillPowerByLevel(std::min(static_cast<int>(SKILL_MAX_LEVEL), m_iLeadership))) / 100.0f;

		switch (bRole)
		{
			case PARTY_ROLE_ATTACKER:
			{
				int iBonus = static_cast<int>(10 + 60.0f * k);
...
void CParty::Update()
{
	...
	for (it = m_memberMap.begin(); it != m_memberMap.end(); ++it)
	{
		...
		if (bLongTimeExpBonusChanged && ch->GetDesc())
			ch->ChatPacket(CHAT_TYPE_INFO, "[LS;726]");
	...
	if (!m_bPartyHealReady)
	{
		...
		m_bCanUsePartyHeal = m_iLeadership >= 18; // ???? 18 ????? ???? ????? ?? ????.
