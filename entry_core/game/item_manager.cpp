...
bool ITEM_MANAGER::Initialize(TItemTable * table, int size)
{
   ...
   std::sort(m_vec_prototype.begin(), m_vec_prototype.end(), [](const TItemTable& a, const TItemTable& b) {
    ...
    for (i = 0; i < size; ++i)
    {
        ...
        if (m_vec_prototype[i].bType == ITEM_QUEST || IS_SET(m_vec_prototype[i].dwFlags, ITEM_FLAG_QUEST_USE | ITEM_FLAG_QUEST_USE_MULTIPLE) || (m_vec_prototype[i].bType == ITEM_COSTUME && m_vec_prototype[i].bSubType == COSTUME_MOUNT))
    ...
    sys_log(1, "ITEM_VID_MAP size: %d", m_VIDMap.size());
    ...
    while (it != m_VIDMap.end())
    {
        ...
        if (NULL == tableInfo)
        {
            sys_err("cannot reset item table for vnum: %u", item->GetOriginalVnum());
            item->SetProto(NULL);
        }
        else
        {
            sys_log(1, "Setting proto for item vnum: %u", item->GetOriginalVnum());
        }

        item->SetProto(tableInfo);
    }

    sys_log(1, "Item table initialized with %d items", size);
    return true;
}
...
{
    sys_log(1, "Searching for item with vnum: %u", vnum);
    ...
    if (rnum < 0)
    {
        for (int i = 0; i < m_vec_item_vnum_range_info.size(); i++)
        {
            ...
            {
                sys_log(1, "Found item in range: %u", p->dwVnum);
                return p;
            }
        }
        sys_log(1, "Item not found for vnum: %u", vnum);
        return NULL;
    }
...
int ITEM_MANAGER::RealNumber(DWORD vnum)
{
    ...
    bot = 0;
    top = m_vec_prototype.size() - 1;

    TItemTable * pTable = &m_vec_prototype[0];

    sys_log(0, "Searching for real number of vnum: %u", vnum);
    sys_log(0, "Prototypes count: %zu", m_vec_prototype.size());

    while (bot <= top)
    {
        mid = (bot + top) >> 1;

        sys_log(0, "Checking position: %d, VNUM: %u", mid, (pTable + mid)->dwVnum);

        if ((pTable + mid)->dwVnum == vnum)
        {
            sys_log(0, "Found vnum %u at position %d", vnum, mid);
            return mid;
        }
    ...
    sys_log(0, "Vnum %u not found", vnum);
    return -1;
}
...
bool ITEM_MANAGER::GetVnum(const char * c_pszName, DWORD & r_dwVnum)
{
    ...
    sys_log(0, "GetVnum called with name: %s", c_pszName);

    for (DWORD i = 0; i < m_vec_prototype.size(); ++i, ++pTable)
    {
        sys_log(0, "Checking item: %s with Vnum: %u", pTable->szLocaleName, pTable->dwVnum);
        if (!strncasecmp(c_pszName, pTable->szLocaleName, len))
        {
            r_dwVnum = pTable->dwVnum;
            sys_log(0, "Found item: %s with Vnum: %u", pTable->szLocaleName, pTable->dwVnum);
            return true;
        }
    }

    sys_log(0, "Item not found: %s", c_pszName);
    return false;
}
