...
void CPetActor::SetName(const char* name)
{
    ...
    if (name && *name)
    {
        petName = std::string(m_pkOwner->GetName()) + " - " + std::string(name);
    }
    else if (m_pkOwner && m_pkOwner->GetName())
    {
        petName = std::string(m_pkOwner->GetName()) + "'s Pet";
    }

    if (IsSummoned())
    {
        m_pkChar->SetName(petName.c_str());
    }

    m_name = std::move(petName);
}
...
void CPetActor::SetSummonItem(LPITEM pItem)
{
    if (pItem)
    {
        pItem->SetSocket(CPetActor::PET_SOCKET_INDEX, true);
...
void CPetActor::UnsetSummonItem()
{
    LPITEM existingSummonItem = ITEM_MANAGER::instance().FindByVID(m_dwSummonItemVID);

    if (existingSummonItem)
    {
        existingSummonItem->SetSocket(CPetActor::PET_SOCKET_INDEX, false);
    }
...
{
    constexpr uint32_t specialVnum1 = 34004;
    constexpr uint32_t specialVnum2 = 34005;

    if ((m_dwVnum == specialVnum1 || m_dwVnum == specialVnum2) && m_pkOwner->GetDungeon() == nullptr)
    {
        return;
    }
...
void CPetSystem::Unsummon(DWORD vnum, bool bDeleteFromList)
{
    ...
    if (0 == actor)
    {
        sys_err("[CPetSystem::Unsummon] Can't find pet by VNUM (%d)", vnum);
        return;
    }
...
void CPetSystem::UnsummonAllPets()
{
	std::for_each(m_petActorMap.begin(), m_petActorMap.end(), [](auto& pair) 
	{
		CPetActor* actor = pair.second;
		if (actor) 
		{
			actor->Unsummon();
		}
	...
	bool bActive = std::any_of(m_petActorMap.begin(), m_petActorMap.end(), [](const auto& pair) 
	{
		return pair.second && pair.second->IsSummoned();
	});
...
bool CPetSystem::HandlePetItem(CItem* item)
{
	...
	if (this->CountSummoned())
	{
		auto* pet = this->GetSummoned();
		if (pet)
		{
			...
			{
				m_pkOwner->SetQuestFlag("pet_system.active_pet_id", 0);
				return false;
			}
		}
	} 
	...
	const std::string_view petName = (pkMob) ? pkMob->GetLocaleName() : "";
	this->Summon(mobVnum, item, petName.data(), bFromFar);

	constexpr std::string_view spawn_effect_file_name = "d:\\ymir work\\effect\\etc\\appear_die\\npc2_appear.mse";
	auto* petActor = this->GetByVnum(mobVnum);
	
	if (petActor && petActor->GetCharacter() && item->GetSubType() == PET_PAY) 
	{
		petActor->GetCharacter()->SpecificEffectPacket(spawn_effect_file_name.data());
	}

	m_pkOwner->SetQuestFlag("pet_system.active_pet_id", item->GetID());
