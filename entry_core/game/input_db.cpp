...
void CInputDB::PlayerLoad(LPDESC d, const char * data)
{
	...
	{
		sys_err("InputDB::PlayerLoad Error: Invalid location for player '%s' (ID: %d). Requested position: MapIndex=%ld, X=%d, Y=%d. Defaulting to empire start position.", 
				pTab->name, pTab->id, pTab->lMapIndex, pTab->x, pTab->y);
...
void CInputDB::SafeboxLoad(LPDESC d, const char * c_pData)
{
	...
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, "[LS;644]");
...
void CInputDB::SafeboxChangePasswordAnswer(LPDESC d, const char* c_pData)
{
	...
	if (p->flag)
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, "[LS;645]");
	}
	else
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, "[LS;646]");
	}
}
...
void CInputDB::GuildRemoveComments(const char* c_pData)
{
    TPacketGuildCommentRemove* p = (TPacketGuildCommentRemove*)c_pData;
    CGuild* g = CGuildManager::instance().TouchGuild(p->dwGuild);

    if (g)
    {
        g->RemoveComments(p->szName);
    }
}
...
void CInputDB::AffectLoad(LPDESC d, const char * c_pData)
{
	...
	uint32_t playerId = decode_4bytes(c_pData);
	c_pData += sizeof(uint32_t);

	uint32_t affectCount = decode_4bytes(c_pData);
	c_pData += sizeof(uint32_t);

	if (ch->GetPlayerID() != playerId)
		return;

	ch->LoadAffect(affectCount, (TPacketAffectElement *) c_pData);
...
void CInputDB::PartyStateChange(const char* c_pData)
{
	...
	sys_log(0, "PartyStateChange: Changed role of PID [%lu] to role [%d] with flag [%d] in party with LeaderPID [%lu]",
			p->dwPID, p->bRole, p->bFlag, p->dwLeaderPID);
...
int CInputDB::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	case HEADER_DG_GUILD_COMMENT_REMOVE:
		GuildRemoveComments(c_pData);
		break;
	...
	case HEADER_DG_REMOVE_ALL_FRIENDS:
		RemoveAllFriends(reinterpret_cast<const char *>(c_pData));
		break;
...
void CInputDB::RemoveAllFriends(const char * c_pData)
{
	const TPacketDGRemoveAllFriends * p = reinterpret_cast<const TPacketDGRemoveAllFriends *>(c_pData);
	MessengerManager::instance().RemoveAllList(p->szPlayerName);
}