...
namespace quest
{
	...
	int32_t item_remove(lua_State* L)
	{
		...
		auto* currentCharacter = questManager.GetCurrentCharacterPtr();

		if (!currentCharacter)
		{
			sys_err("[ItemRemove] No character found for item removal.");
			return 0;
		}

		auto* currentItem = questManager.GetCurrentItem();
		
		if (!currentItem || !currentItem->GetOwner()) 
		{
			sys_err("[ItemRemove] No item to remove or item does not have an owner.");
			return 0;
		}

		if (currentCharacter != currentItem->GetOwner()) 
		{
			sys_err("[ItemRemove] Character %u attempted to remove item %p not owned by them.", currentCharacter->GetPlayerID(), static_cast<void*>(currentItem));
			return 0;
		}

		ITEM_MANAGER::Instance().RemoveItem(currentItem);
		questManager.ClearCurrentItem();

		sys_log(0, "[ItemRemove] Item %p removed successfully by character %u.", static_cast<void*>(currentItem), currentCharacter->GetPlayerID());
