...
void CHARACTER::DeathPenalty(BYTE bTown)
{
	...
	if (GetLevel() < 10)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, "[LS;275]");
		return;
	}
...
void CHARACTER::ItemDropPenalty(LPCHARACTER pkKiller)
{
	...
	if (bDropInventory) // Drop Inventory
	{
		...
		const bool isQuestActive = quest::CQuestManager::instance().GetPCForce(GetPlayerID())->IsRunning();

		for (uint32_t slotIndex = 0; slotIndex < INVENTORY_MAX_NUM; ++slotIndex)
		{
			auto currentItem = GetInventoryItem(slotIndex);

			if (!currentItem || (isQuestActive && currentItem->GetType() == ITEM_QUEST)) 
			{
				continue;
			}

			vec_bSlots.emplace_back(slotIndex);
		}
	...
	if (bDropAntiDropUniqueItem)
	{
		...
		if (pkItem && pkItem->GetVnum() == UNIQUE_ITEM_SKIP_ITEM_DROP_PENALTY)
		{
			SyncQuickslot(QUICKSLOT_TYPE_ITEM, WEAR_COSTUME_MOUNT, UCHAR_MAX);
			vec_item.push_back(std::make_pair(pkItem->RemoveFromCharacter(), EQUIPMENT));
		}
#endif
	}
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	else if (GetMountVnum())
	{
		...
		const std::array<int, 3> wearSlots = {WEAR_UNIQUE1, WEAR_UNIQUE2, WEAR_COSTUME_MOUNT};
		
		for (int slot : wearSlots)
		{
			LPITEM item = GetWear(slot);
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int32_t dam, EDamageType type)
{
	...
	{
		dam = CalculateMagicDamage(dam, pAttacker->GetPoint(POINT_MAGIC_ATT_BONUS_PER), pAttacker->GetPoint(POINT_MELEE_MAGIC_ATT_BONUS_PER));
	}
	
	constexpr int32_t MAX_PROBABILITY = 100;
	constexpr double TERROR_EFFECT_DIVISOR = 3.3;

	if (IsAffectFlag(AFF_TERROR))
	{
		if (type != DAMAGE_TYPE_NORMAL && type != DAMAGE_TYPE_NORMAL_RANGE)
		{
			int32_t terrorEffectProbability = static_cast<int32_t>(GetSkillPower(SKILL_TERROR) / TERROR_EFFECT_DIVISOR);

			if (number(1, MAX_PROBABILITY) <= terrorEffectProbability)
			{
	...
	else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
	{
		...
		if (pAttacker)
		{
			...
			int32_t iPenetratePct = CalculatePenetrationPercent(pAttacker->GetPoint(POINT_PENETRATE_PCT), IsPC(), pAttacker->GetMarriageBonus(UNIQUE_ITEM_MARRIAGE_PENETRATE_BONUS), GetSkillPower(SKILL_RESIST_PENETRATE), GetPoint(POINT_RESIST_PENETRATE));

			auto [newDam, IsPenetrate] = CalculatePenetrationDamage(iPenetratePct, dam, test_server, GetPoint(POINT_DEF_GRADE), GetPoint(POINT_DEF_BONUS));
			dam = newDam;
			
			/* ### Refactored Code ~ Sevence
			 * - Replaced hard-coded probability checks with dynamic checks based on the POINT_STEAL_HP attribute.
			 * - Added a check to ensure that the attacker's HP does not exceed the maximum HP limit.
			 * - Used std::clamp to limit the damage value used in HP calculation.
			 * - Changed the flying effect to FLY_HP_MEDIUM. */
			 
			auto stealHPAttribute = pAttacker->GetPoint(POINT_STEAL_HP);

			if (stealHPAttribute)
			{
				if (number(1, MAX_PROBABILITY) <= stealHPAttribute)
				{
					auto iHP = std::clamp(dam, 0, GetHP()) * stealHPAttribute / 100;

					if (iHP > 0 && GetHP() >= iHP && (pAttacker->GetHP() + iHP) < pAttacker->GetMaxHP())
					{
			...
			/* ### Refactored Code ~ Sevence
			 * - Replaced hard-coded probability checks with dynamic checks based on the POINT_STEAL_SP attribute.
			 * - Added a check to ensure that the attacker's SP does not exceed the maximum SP limit.
			 * - Used std::clamp to limit the damage value used in SP calculation.
			 * - Changed the flying effect from FLY_SP_SMALL to FLY_SP_MEDIUM.
			 * 
			 * ### Bug Fixes
			 * - Fixed an issue where SP could exceed the maximum limit. */
			...
			if (stealSPAttribute)
			{
				if (number(1, MAX_PROBABILITY) <= stealSPAttribute)
				{
					...
					int32_t iSP = std::clamp(dam, 0, iCur) * stealSPAttribute / 100;
			...
			if (pAttacker->GetPoint(POINT_HIT_HP_RECOVERY) > 0 && number(0, 4) > 0)
			{
				int32_t recoveryRatio = std::clamp(pAttacker->GetPoint(POINT_HIT_HP_RECOVERY), 0, 100);
				int32_t hpRecoveryAmount = std::min(dam, iCurHP) * recoveryRatio / 100;

				if (hpRecoveryAmount > 0)
				{
					CreateFly(FLY_HP_SMALL, pAttacker);
					pAttacker->PointChange(POINT_HP, hpRecoveryAmount);
				}
			}

			if (pAttacker->GetPoint(POINT_HIT_SP_RECOVERY) > 0 && number(0, 4) > 0)
			{
				int32_t recoveryRatio = std::clamp(pAttacker->GetPoint(POINT_HIT_SP_RECOVERY), 0, 100);
				int32_t spRecoveryAmount = std::min(dam, iCurHP) * recoveryRatio / 100;

				if (spRecoveryAmount > 0)
				{
					CreateFly(FLY_SP_SMALL, pAttacker);
					pAttacker->PointChange(POINT_SP, spRecoveryAmount);
				}
			}
...
void CHARACTER::AggregateMonster()
{
	...
	if (pSec)
	{
		...
		pSec->ForEachAround(f);
		EffectPacket(SE_EFFECT_CAPE_OF_COURAGE);
	}
}
