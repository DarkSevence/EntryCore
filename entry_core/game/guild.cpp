...
CGuild::CGuild(TGuildCreateParameter & cp)
{
	...
	strlcpy(m_data.grade_array[0].grade_name, "[LS;773]", sizeof(m_data.grade_array[0].grade_name));
	...
	{
		strlcpy(m_data.grade_array[i].grade_name, "[LS;774]", sizeof(m_data.grade_array[i].grade_name));
...
bool CGuild::OfferExp(LPCHARACTER ch, int amount)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;608]");
		return false;
	}
...
void CGuild::Disband()
{
	...
	for (TGuildMemberOnlineContainer::iterator it = m_memberOnline.begin(); it != m_memberOnline.end(); ++it)
	{
		...
		ch->SetQuestFlag("guild_manager.guild_reformation_cooldown", get_global_time());
	}
...
void CGuild::AddComment(LPCHARACTER character, const std::string& comment)
{
	...
	{
		character->ChatPacket(CHAT_TYPE_INFO, "[LS;304]");
		return;
	}
	...
	if (now - lastCommentTime < FloodProtectionInterval) 
	{
		...
		character->ChatPacket(CHAT_TYPE_INFO, "[LS;303;%lld]", remainingTime);
		return;
	}

	lastCommentTime = now;

	if (comment.length() > GUILD_COMMENT_MAX_LEN || comment.length() == 0) 
	{
		character->ChatPacket(CHAT_TYPE_INFO, "[LS;305]");
		return;
	}

	char escapedComment[GUILD_COMMENT_MAX_LEN * 2 + 1];
	DBManager::instance().EscapeString(escapedComment, sizeof(escapedComment), comment.c_str(), comment.length());
	
	auto query = fmt::format("INSERT INTO guild_comment{} (guild_id, name, notice, content, time) VALUES ('{}', '{}', '{}', '{}', NOW())", 
		get_table_postfix(), m_data.guild_id, character->GetName(), (comment[0] == '!') ? "1" : "0", escapedComment);
...
void CGuild::DeleteComment(LPCHARACTER ch, DWORD comment_id)
{
	...
	if (pmsg->Get()->uiAffectedRows == 0 || pmsg->Get()->uiAffectedRows == (uint32_t)-1)
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;609]");
	else
		RefreshCommentsForAllMembers(); 

	M2_DELETE(pmsg);
}

void CGuild::RemoveComments(const char* playerName)
{
    DBManager::instance().Query("DELETE FROM guild_comment%s WHERE guild_id=%u AND name='%s'", get_table_postfix(), GetID(), playerName);
	RefreshCommentsForAllMembers(); 
}

void CGuild::RefreshCommentsForAllMembers()
{
    for (auto& member : m_memberOnline)
    {
        if (member)
        {
            RefreshCommentForce(member->GetPlayerID());
        }
    }
}
...
void CGuild::RefreshCommentForce(DWORD player_id)
{
    ...
    if (ch == NULL) {
        sys_err("RefreshCommentForce: Failed to find character with playerID: %u", player_id);
        return;
    }
    ...
    if (!d) 
    {
        sys_err("RefreshCommentForce: Failed to get descriptor for playerID: %u", player_id);
        return;
    }

    pack.size += (sizeof(DWORD) + CHARACTER_NAME_MAX_LEN + 1 + GUILD_COMMENT_MAX_LEN + 1) * count;
    d->BufferedPacket(&pack, sizeof(pack));
    ...
    for (uint i = 0; i < pmsg->Get()->uiNumRows; i++)
    {
        ...
        if (i == pmsg->Get()->uiNumRows - 1)
            d->Packet(szContent, sizeof(szContent)); // If it's the last row, send it
        else
            d->BufferedPacket(szContent, sizeof(szContent));
    }

    sys_log(0, "RefreshCommentForce: Refreshed comments for playerID: %u, guildID: %u", player_id, m_data.guild_id);
}
...
void CGuild::UseSkill(DWORD dwVnum, LPCHARACTER ch, DWORD pid)
{
	...
	if (GetSP() < iNeededSP)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;610;%d;%d]", GetSP(), iNeededSP);
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;611]");
		return;
	}
		...
		case GUILD_SKILL_TELEPORT:
			...
			else
			{
				if (m_memberP2POnline.find(pid) != m_memberP2POnline.end())
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "[LS;613;%d;%d]", pcci->bChannel, g_bChannel);
					}
					else
					{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, "[LS;614]");
			}
			break;
		...
		default:
			{
				...
				if (!UnderAnyWar())
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "[LS;615]");
					return;
				}
...
bool CGuild::ChargeSP(LPCHARACTER ch, int iSP)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;616;%u]", iSP);
	}
	return true;
}
...
void CGuild::RequestWithdrawMoney(LPCHARACTER ch, int iGold)
{
	if (false==ch->CanDeposit())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;618]");
		return;
	}

	if (ch->GetPlayerID() != GetMasterPID())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;617]");
		return;
	}

	if (m_data.gold < iGold)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;619]");
		return;
	}
...
void CGuild::Invite( LPCHARACTER pchInviter, LPCHARACTER pchInvitee )
{
	...
	{
	    pchInviter->ChatPacket(CHAT_TYPE_INFO, "[LS;620]");
	    return;
	}
	...
	switch (errcode)  
	{
		...
		case GERR_WITHDRAWPENALTY: pchInviter->ChatPacket( CHAT_TYPE_INFO, "[LS;624;%d]", quest::CQuestManager::instance().GetEventFlag("member_rejoin_cooldown_delay")); return;
		case GERR_COMMISSIONPENALTY: pchInviter->ChatPacket( CHAT_TYPE_INFO, "[LS;625;%d]", quest::CQuestManager::instance().GetEventFlag("guild_reformation_cooldown_delay")); return;
		case GERR_ALREADYJOIN:	pchInviter->ChatPacket(CHAT_TYPE_INFO, "[LS;626]"); return;
		case GERR_GUILDISFULL:	pchInviter->ChatPacket(CHAT_TYPE_INFO, "[LS;627]"); return;
		case GERR_GUILD_IS_IN_WAR : pchInviter->ChatPacket( CHAT_TYPE_INFO, "[LS;628]"); return;
		case GERR_INVITE_LIMIT : pchInviter->ChatPacket( CHAT_TYPE_INFO, "[LS;629]"); return;
	...
	// Send invite packet to invitee character
    DWORD gid = GetID();
    const char *inviterName = pchInviter->GetName();
    const char *guildName = GetName();
    ...
    buf.write(guildName, GUILD_NAME_MAX_LEN + 1);
...
void CGuild::InviteAccept( LPCHARACTER pchInvitee )
{
	...
	switch ( errcode ) 
	{
		...
		case GERR_COMMISSIONPENALTY:
						...
						return;
		case GERR_ALREADYJOIN:	pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;626]"); return;
		case GERR_GUILDISFULL:	pchInvitee->ChatPacket(CHAT_TYPE_INFO, "[LS;627]"); return;
		case GERR_GUILD_IS_IN_WAR : pchInvitee->ChatPacket( CHAT_TYPE_INFO, "[LS;628]"); return;
		case GERR_INVITE_LIMIT : pchInvitee->ChatPacket( CHAT_TYPE_INFO, "[LS;629]"); return;
...
{
	if (get_global_time() - pchInvitee->GetQuestFlag("guild_manager.member_rejoin_cooldown") < CGuildManager::instance().GetWithdrawDelay()) return GERR_WITHDRAWPENALTY;
	else if (get_global_time() - pchInvitee->GetQuestFlag("guild_manager.guild_reformation_cooldown") < CGuildManager::instance().GetDisbandDelay()) return GERR_COMMISSIONPENALTY;
	else if (pchInvitee->GetGuild()) return GERR_ALREADYJOIN;
