...
void SendBlockChatInfo(LPCHARACTER ch, int sec)
{
	if (sec <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;72]");
		return;
	}
	...
	if (hour > 0 && min > 0)
		snprintf(buf, sizeof(buf), "[LS;651;%d;%d;%d]", hour, min, sec);
	else if (hour > 0 && min == 0)
		snprintf(buf, sizeof(buf), "[LS;652;%d;%d]", hour, sec);
	else if (hour == 0 && min > 0)
		snprintf(buf, sizeof(buf), "[LS;653;%d;%d]", min, sec);
	else
		snprintf(buf, sizeof(buf), "[LS;654;%d]", sec);
...
int CInputMain::Whisper(LPCHARACTER ch, const char * data, size_t uiBytes)
{
    ...
    {
        int remainingTime = ch->GetRemainingBlockTime();
        if (remainingTime == 0)
        {
            ch->SetChatBlockTime(10);
        }
        else
        {
			ch->ChatPacket(CHAT_TYPE_INFO, "[LS;780;%d]", remainingTime);
            return iExtraLen;
        }
    }
    ...
    if (ch->IsChatBlocked())
    {
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;780;%d]", ch->GetRemainingBlockTime());
        return iExtraLen;
    }
...
void CInputMain::SendWhisperError(LPCHARACTER ch, const char* format, ...)
{
    if (ch && ch->GetDesc())
    {
        char message[CHAT_MAX_LEN + 1];
        va_list args;
        ...
        pack.bHeader = HEADER_GC_WHISPER;
        pack.bType = WHISPER_TYPE_ERROR;
        pack.wSize = sizeof(TPacketGCWhisper) + strlen(message);
        strlcpy(pack.szNameFrom, "SYSTEM", sizeof(pack.szNameFrom));

        TEMP_BUFFER tmpbuf;
        tmpbuf.write(&pack, sizeof(pack));
        tmpbuf.write(message, strlen(message));
...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (ch->IncreaseChatCounter() >= 10 && !ch->IsGM())
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "[LS;781;%d]", remainingTime);
			return iExtraLen;
		}
	}
	...
	if (ch->IsChatBlocked())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;781;%d]", ch->GetRemainingBlockTime());
		return iExtraLen;
	}
	...
	if (0!=processReturn)
	{
		...
		{
			if (3==processReturn) //교환중
				ch->ChatPacket(CHAT_TYPE_INFO, "[LS;655]", pTable->szLocaleName);
			else
				ch->ChatPacket(CHAT_TYPE_INFO, "[LS;656;%s]", pTable->szLocaleName);
	...
	if (pinfo->type == CHAT_TYPE_SHOUT)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "[LS;657;%d]", SHOUT_LIMIT_LEVEL);
			return (iExtraLen);
		}
	...
	switch (pinfo->type)
	{
		...
		case CHAT_TYPE_PARTY:
			{
				if (!ch->GetParty())
					ch->ChatPacket(CHAT_TYPE_INFO, "[LS;658]");
				else
				{
					TEMP_BUFFER tbuf;
		...
		case MESSENGER_SUBHEADER_CG_ADD_BY_VID:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "[LS;660]");
					return sizeof(TPacketCGMessengerAddByVID);
				}
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "[LS;661]");
					return sizeof(TPacketCGMessengerAddByVID);
				}
		...
		case MESSENGER_SUBHEADER_CG_ADD_BY_NAME:
			{
				...
				if (!tch)
					ch->ChatPacket(CHAT_TYPE_INFO, "[LS;662;%s]", name);
				else
				{
...
void CInputMain::Exchange(LPCHARACTER ch, const char * data)
{
	...
	if ((to_ch = CHARACTER_MANAGER::instance().Find(pinfo->arg1)))
	{
		...
		{
			to_ch->ChatPacket(CHAT_TYPE_INFO, "[LS;663;%d]", g_nPortalLimitTime);
			return;
		}
	...
	switch (pinfo->sub_header)
	{
		case EXCHANGE_SUBHEADER_CG_START:	// arg1 == vid of target character
			if (!ch->GetExchange())
			{
				if ((to_ch = CHARACTER_MANAGER::instance().Find(pinfo->arg1)))
				{
					...
					if (iPulse - to_ch->GetSafeboxLoadTime() < PASSES_PER_SEC(g_nPortalLimitTime))
					{
						to_ch->ChatPacket(CHAT_TYPE_INFO, "[LS;664;%d]", g_nPortalLimitTime);
		...
		case EXCHANGE_SUBHEADER_CG_ITEM_ADD:
			if (ch->GetExchange()) {
				if (!ch->GetExchange()->GetCompany()->GetAcceptStatus())
				{
					ch->GetExchange()->AddItem(pinfo->Pos, pinfo->arg2, pinfo->isRightClickAutoMoveEnabled);
				}
			}
			break;
...
void CInputMain::SafeboxCheckin(LPCHARACTER ch, const char* c_pData)
{
	...
	const bool isQuestRunning = (pc != nullptr) && pc->IsRunning();
	...
	auto* packetData = (TPacketCGSafeboxCheckin*)c_pData;
	...
	auto* currentPlayerSafebox = ch->GetSafebox();
	auto inventoryItem = ch->GetItem(packetData->ItemPos);

	if (!currentPlayerSafebox || !inventoryItem)
	{
		return;
	}

	if (packetData->isRightClickAutoMoveEnabled)
	{
		int32_t AutoPos = currentPlayerSafebox->GetEmptySafebox(inventoryItem->GetSize());
		...
		packetData->bSafePos = AutoPos;
	}

	if (inventoryItem->GetType() == ITEM_BELT && inventoryItem->IsEquipped() && CBeltInventoryHelper::IsExistItemInBeltInventory(ch))
	{
	...
	const auto [cell, flag] = std::tuple{inventoryItem->GetCell(), inventoryItem->GetFlag()};
	if (cell >= INVENTORY_MAX_NUM && IS_SET(flag, ITEM_FLAG_IRREMOVABLE))
	{
	    ch->ChatPacket(CHAT_TYPE_INFO, "[LS;669]");
	    return;
	}

	if (!currentPlayerSafebox->IsEmpty(packetData->bSafePos, inventoryItem->GetSize()))
	{
	    ch->ChatPacket(CHAT_TYPE_INFO, "[LS;670]");
		return;
	}
	...
	if (inventoryItem->GetVnum() == UNIQUE_ITEM_SAFEBOX_EXPAND || IS_SET(inventoryItem->GetAntiFlag(), ITEM_ANTIFLAG_SAFEBOX) || inventoryItem->isLocked())
	{
	    ch->ChatPacket(CHAT_TYPE_INFO, "[LS;669]");
		return;
	}

	inventoryItem->RemoveFromCharacter();

	if (!inventoryItem->IsDragonSoul())
	{
		ch->SyncQuickslot(QUICKSLOT_TYPE_ITEM, packetData->ItemPos.cell, 255);
	}

	currentPlayerSafebox->Add(packetData->bSafePos, inventoryItem);

	std::array<char, 128> szHint{};
	std::snprintf(szHint.data(), szHint.size(), "%s %u", inventoryItem->GetName(), inventoryItem->GetCount());
	LogManager::instance().ItemLog(ch, inventoryItem, "SAFEBOX PUT", szHint.data());
}
...
void CInputMain::SafeboxCheckout(LPCHARACTER ch, const char * c_pData, bool bMall)
{
	...
	if (p->isRightClickAutoMoveEnabled)
	{
		int32_t AutoPos = pkItem->IsDragonSoul() ? ch->GetEmptyDragonSoulInventory(pkItem) : ch->GetEmptyInventory(pkItem->GetSize());
	...
	else
	{
		...
		if (p->ItemPos.IsBeltInventoryPosition() && !CBeltInventoryHelper::CanMoveIntoBeltInventory(pkItem))
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("CANNOT_MOVE_ITEM_BELT"));
			return;
		}
...
void CInputMain::PartySetState(LPCHARACTER ch, const char* c_pData)
{
    ...
    DWORD pid = p->pid;
    sys_err("PARTY SetRole pid %d to role %d state %s", pid, p->byRole, p->flag ? "on" : "off");

    switch (p->byRole)
    {
        case PARTY_ROLE_NORMAL:
            sys_err("PartySetState: Setting role to NORMAL for PID %d", pid);
            break;
        ...
        case PARTY_ROLE_DEFENDER:
            if (ch->GetParty()->SetRole(pid, p->byRole, p->flag))
            {
                ...
                sys_err("PartySetState: Successfully set role %d to %s for PID %d", p->byRole, p->flag ? "on" : "off", pid);
            }
            else
            {
                sys_err("PartySetState: Failed to set role %d to %s for PID %d", p->byRole, p->flag ? "on" : "off", pid);
            }
            break;
...
void CInputMain::AnswerMakeGuild(LPCHARACTER ch, const char* c_pData)
{
	...
	if (get_global_time() - ch->GetQuestFlag("guild_manager.guild_reformation_cooldown") <
			CGuildManager::instance().GetDisbandDelay())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS:677;%d]",
				quest::CQuestManager::instance().GetEventFlag("guild_reformation_cooldown_delay"));
		return;
	}

	if (get_global_time() - ch->GetQuestFlag("guild_manager.member_rejoin_cooldown") <
			CGuildManager::instance().GetWithdrawDelay())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;678;%d]",
				quest::CQuestManager::instance().GetEventFlag("member_rejoin_cooldown_delay"));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;679]");
		return;
	}
	...
	if (dwGuildID)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;680;%s]", cp.name);
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;681]");
}
...
void CInputMain::PartyUseSkill(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;682]");
		return;
	}
...
int CInputMain::Guild(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	switch (SubHeader)
	{
		...
		{
			using VidType = const uint32_t;
			auto vid = *reinterpret_cast<VidType*>(c_pData);
			auto newmember = CHARACTER_MANAGER::instance().Find(vid);
		...
		case GUILD_SUBHEADER_CG_REMOVE_MEMBER:
			{
				...
				if (member)
				{
					...
					member->SetQuestFlag("guild_manager.member_rejoin_cooldown", get_global_time());
				...
				else
				{
					...
					if (pGuild->RequestRemoveMember(pid))
						ch->ChatPacket(CHAT_TYPE_INFO, "[LS;691]");
					else
						ch->ChatPacket(CHAT_TYPE_INFO, "[LS;688]");
				}
			}
		...
		case GUILD_SUBHEADER_CG_CHANGE_GRADE_NAME:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "[LS;692]");
				}
		...
		case GUILD_SUBHEADER_CG_OFFER:
			{
				...
				else
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "[LS;698;%u]", offer);
					}
					else
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "[LS;699]");
					}
				}
			}
			return SubPacketLen;
		...
		case GUILD_SUBHEADER_CG_CHANGE_MEMBER_GRADE:
			{
				...
				if (m->grade != GUILD_LEADER_GRADE)
					ch->ChatPacket(CHAT_TYPE_INFO, "[LS;703]");
...
int CInputMain::MyShop(LPCHARACTER ch, const char * c_pData, size_t uiBytes)
{
	...
	if (ch->GetGold() >= GOLD_MAX)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;24]");
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;30]");
		return (iExtraLen);
	}
	...
	else if (p->type == REFINE_TYPE_MONEY_ONLY)
	{
		...
		if (NULL != item)
		{
			...
			else
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "[LS;190]");
				}
			}
		}
	}
		...
		case HEADER_CG_MOVE:
			...
			if (LC_IsEurope())
			{
				if (g_bCheckClientVersion)
				{
					...
					if (version > date)
					{
						ch->ChatPacket(CHAT_TYPE_NOTICE, "[LS;562]");
