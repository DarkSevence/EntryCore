...
{
    return quest::CQuestManager::instance().GetEventFlag("guild_reformation_cooldown_delay") * 86400;
}

int CGuildManager::GetWithdrawDelay()
{
    return quest::CQuestManager::instance().GetEventFlag("member_rejoin_cooldown_delay") * 86400;
}
...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
	...
	{
		gcp.master->ChatPacket(CHAT_TYPE_INFO, "[LS;89]");
		return 0;
	}
	...
	if (pmsg->Get()->uiNumRows > 0)
	{
		...
		if (!(row[0] && row[0][0] == '0'))
		{
			gcp.master->ChatPacket(CHAT_TYPE_INFO, "[LS;630]");
			return 0;
		}
	}
	else
	{
		gcp.master->ChatPacket(CHAT_TYPE_INFO, "[LS;631]");
		return 0;
	}
...
void CGuildManager::DeclareWar(DWORD guild_id1, DWORD guild_id2, BYTE bType)
{
		...
		g2->DeclareWar(guild_id1, bType, GUILD_WAR_RECV_DECLARE))
	{
		...
		{
			char buf[256];
			snprintf(buf, sizeof(buf), "[LS;632;%s;%s]", TouchGuild(guild_id1)->GetName(), TouchGuild(guild_id2)->GetName());
			SendNotice(buf);
		}
	}
}
...
void CGuildManager::RefuseWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1 && g2)
	{
		if (g2->GetMasterCharacter())
			g2->GetMasterCharacter()->ChatPacket(CHAT_TYPE_INFO, "[LS;633;%s;%s]", g1->GetName(), g2->GetName());
	}
...
void CGuildManager::WaitStartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	{
		char buf[256];
		snprintf(buf, sizeof(buf), "[LS;634;%s;%s]", g1->GetName(), g2->GetName());
		SendNotice(buf);
	}
}
...
void CGuildManager::StartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	char buf[256];
	snprintf(buf, sizeof(buf), "[LS;635;%s;%s]", g1->GetName(), g2->GetName());
	SendNotice(buf);
...
void SendGuildWarOverNotice(CGuild* g1, CGuild* g2, bool bDraw)
{
	if (g1 && g2)
	{
		...
		const char* name1 = g1->GetName();
		const char* name2 = g2->GetName();

		if (name1 && name2)
		{
			if (bDraw)
			{
				snprintf(buf, sizeof(buf), "[LS;636;%s;%s]", name1, name2);
			}
			else
			{
				if (g1->GetWarScoreAgainstTo(g2->GetID()) > g2->GetWarScoreAgainstTo(g1->GetID()))
				{
					snprintf(buf, sizeof(buf), "[LS;637;%s;%s]", name1, name2);
				}
				else
				{
					snprintf(buf, sizeof(buf), "[LS;637;%s;%s]", name2, name1);
				}
			}
...
void CGuildManager::CancelWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1)
	{
		...
		if (master1)
			master1->ChatPacket(CHAT_TYPE_INFO, "[LS;638]");
	}
	...
	if (g2)
	{
		...
		if (master2)
			master2->ChatPacket(CHAT_TYPE_INFO, "[LS;638]");
	}

	if (g1 && g2)
	{
		char buf[256+1];
		snprintf(buf, sizeof(buf), "[LS;633;%s;%s]", g1->GetName(), g2->GetName());
		SendNotice(buf);
	}
}
