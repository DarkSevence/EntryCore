...
bool ITEM_MANAGER::ReadEtcDropItemFile(const char * c_pszFileName)
{
	...
	while (fgets(buf, 512, fp))
	{
		...
		*cpTab = '\0';
		...
		bool itemFound = ITEM_MANAGER::instance().GetVnumByOriginalName(szItemName, dwItemVnum);

		if (!itemFound) 
		{
			itemFound = str_to_number(dwItemVnum, szItemName) && ITEM_MANAGER::instance().GetTable(dwItemVnum);
		}

		if (!itemFound) 
		{
			sys_err("ETC_DROP_ITEM: Failed to find the item. Searched by name and Vnum. Given identifier: '%s'", szItemName);
			fclose(fp);
			return false;
		}
