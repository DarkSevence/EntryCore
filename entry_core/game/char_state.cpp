						 ...
						 pkChr->IsAffectFlag(AFF_WAR_FLAG3)))
				{
					...
					if (pkAff)
					{
						if ((DWORD) m_pkChr->GetPoint(POINT_STAT) == pkGuild->GetID() &&
								m_pkChr->GetPoint(POINT_STAT) != pkAff->applyValue)
						{
							CWarMap * pMap = pkChr->GetWarMap();
							BYTE idx;
							...
							//if (pMap->IsFlagOnBase(idx))
							{
								...
								SendGuildWarScore(m_pkChr->GetPoint(POINT_STAT), pkAff->applyValue, 1);

								pMap->ResetFlag();

								char buf[256];
								snprintf(buf, sizeof(buf), "[LS;778;%s;%s]", pMap->GetGuild(idx)->GetName(), pMap->GetGuild(idx_opp)->GetName());
								pMap->Notice(buf);
							}
						}
					}
...
void CHARACTER::StateFlag()
{
	...
	snprintf(buf, sizeof(buf), "[LS;778;%s;%s]", pMap->GetGuild(idx)->GetName(), f.m_pkChrFind->GetName());
	pMap->Notice(buf);
}
