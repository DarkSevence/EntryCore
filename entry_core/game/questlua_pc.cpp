...
namespace quest 
{
	...
	int32_t pc_set_skillgroup(lua_State* L)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("Failed in pc_set_skillgroup: Invalid argument. Expected a number for skill group.");
			return 0;
		}
		
		CQuestManager& questManager = CQuestManager::Instance();
		LPCHARACTER currentCharacter = questManager.GetCurrentCharacterPtr();
		
		if (currentCharacter == nullptr)
		{
			sys_err("Failed in pc_set_skillgroup: The current character pointer is null.");
			return 0;
		}
		
		const int32_t skillGroup = static_cast<int32_t>(std::lrint(lua_tonumber(L, 1)));
		currentCharacter->RemoveSkillAffects();
		currentCharacter->SetSkillGroup(static_cast<uint8_t>(skillGroup));
	...
	int32_t pc_select_vid(lua_State* L)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("pc_select_vid: Expected number as first argument.");
			lua_pushnumber(L, 0);
			return 1;
		}
		
		uint32_t targetVid = static_cast<uint32_t>(lua_tonumber(L, 1));
		
		CHARACTER* currentCharacter = CQuestManager::Instance().GetCurrentCharacterPtr();

		if (!currentCharacter)
		{
			sys_err("pc_select_vid: Null character pointer encountered. File: %s, Line: %d", __FILE__, __LINE__);
			lua_pushnumber(L, 0);
			return 1;
		}
		
		CHARACTER* targetCharacter = CHARACTER_MANAGER::Instance().Find(targetVid);
		
		if (targetCharacter)
		{
			CQuestManager::Instance().GetPC(targetCharacter->GetPlayerID());
			lua_pushnumber(L, static_cast<uint32_t>(currentCharacter->GetVID()));
			return 1;
		}
...
teleport_area:
	...
	enum class ChangeNameResult : int32_t 
	{
		Success = 0,
		NameAlreadyInUse,
		InvalidParameter,
	...
	int32_t pc_change_name(lua_State* luaState)
	{
		auto* currentCharacter = CQuestManager::instance().GetCurrentCharacterPtr();
		
		if (!currentCharacter) 
		{
			sys_err("Null pointer for currentCharacter in function pc_change_name at %s:%d", __FILE__, __LINE__);
			lua_pushnumber(luaState, static_cast<int32_t>(ChangeNameResult::InvalidParameter));
			return 1;
		}
		
		if (!currentCharacter->GetNewName().empty()) 
		{
			sys_err("Attempt to change to an already used name in function pc_change_name at %s:%d", __FILE__, __LINE__);
			lua_pushnumber(luaState, static_cast<int32_t>(ChangeNameResult::NameAlreadyInUse));
			return 1;
		}		

		if (!lua_isstring(luaState, 1)) 
		{
			sys_err("Invalid parameter type for new name in function pc_change_name at %s:%d", __FILE__, __LINE__);
			lua_pushnumber(luaState, static_cast<int32_t>(ChangeNameResult::InvalidParameter));
			return 1;
		}

		const char* newPlayerName = lua_tostring(luaState, 1);

		if (!check_name(newPlayerName)) 
		{
			sys_err("Illegal characters found in new name in function pc_change_name at %s:%d", __FILE__, __LINE__);
			lua_pushnumber(luaState, static_cast<int32_t>(ChangeNameResult::IllegalName));
			return 1;
		}

		auto query = fmt::format("SELECT COUNT(*) FROM player WHERE name='{}'", newPlayerName);
		auto sqlMessage = std::unique_ptr<SQLMsg>(DBManager::Instance().DirectQuery(query.c_str()));
	
		if (sqlMessage->Get()->uiNumRows > 0)
		{
			MYSQL_ROW sqlRow = mysql_fetch_row(sqlMessage->Get()->pSQLResult);
			int32_t existingNamesCount = std::stoi(sqlRow[0]);

			if (existingNamesCount != 0) 
			{
				sys_err("New name already exists in database in function pc_change_name at %s:%d", __FILE__, __LINE__);
				lua_pushnumber(luaState, static_cast<int32_t>(ChangeNameResult::NameAlreadyInUse));
				return 1;
			}
		}

		uint32_t pid = currentCharacter->GetPlayerID();
		...
		LogManager::Instance().ChangeNameLog(pid, currentCharacter->GetName(), newPlayerName, currentCharacter->GetDesc()->GetHostName());

		auto updateQuery = fmt::format("UPDATE player SET name='{}' WHERE id={}", newPlayerName, pid);
		auto pmsg2 = std::unique_ptr<SQLMsg>(DBManager::Instance().DirectQuery(updateQuery.c_str()));
		
		currentCharacter->SetNewName(newPlayerName);
		lua_pushnumber(luaState, static_cast<int32_t>(ChangeNameResult::Success));
		return 1;
	}
	...
	int pc_get_special_ride_vnum(lua_State* L)
	{
		...
		if (NULL != pChar)
		{
			...
			LPITEM MountCostumeSlot = pChar->GetWear(WEAR_COSTUME_MOUNT);
			...
			if (MountCostumeSlot)
			{
				lua_pushnumber(L, MountCostumeSlot->GetVnum());
				lua_pushnumber(L, MountCostumeSlot->GetSocket(0)); // @fixme152 (2->0)
				return 2;
			}
