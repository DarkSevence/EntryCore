...
static uint32_t GetLastPlayTime(uint32_t id)
{
    std::stringstream query;
    query << "SELECT UNIX_TIMESTAMP(last_play) FROM player" << GetTablePostfix() << " WHERE id=" << id;

    std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(query.str().c_str(), SQL_PLAYER));
    auto sResult = pMsg->Get();
