...
class CClientManager : public CNetBase, public singleton<CClientManager>
{
	...
	void SendGuildRemoveMemberPacket(DWORD guildID, DWORD playerID);
	DWORD GetGuildIDByPlayerID(DWORD playerID);
	const char* GetCharacterName(DWORD playerID);
	...
	void		RESULT_AFFECT_LOAD(CPeer * pkPeer, MYSQL_RES * pRes, DWORD dwHandle, DWORD dwClientPID);
	...
	std::string GetPlayerNameByID(DWORD playerID);
	void SendRemoveAllFriendsPacket(const char* playerName);
	 ...
	 */
	void MyshopPricelistUpdate(const TItemPriceListTable* packet);
