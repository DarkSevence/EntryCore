...
#ifdef ENABLE_PROTO_FROM_DB
	extern bool bEnableDatabaseMirroring;
	if (bEnableDatabaseMirroring)
	{
...
bool CClientManager::InitializeItemTableFromDB()
{
	...
	sys_log(0, "Query for InitializeItemTableFromDB: %s", query);
	...
	while ((data = mysql_fetch_row (pRes->pSQLResult)))
	{
		...
		sys_log(0, "Loaded item: vnum=%d name=%s", item_table->dwVnum, item_table->szName);
		++item_table;
	}

	sort (m_vec_itemTable.begin(), m_vec_itemTable.end(), FCompareVnum());
	sys_log(0, "ITEM_PROTO loaded successfully.");
