...
#if defined(ENABLE_PROTO_FROM_DB)
	bool bEnableDatabaseMirroring = false;
#endif
...
bool CClientManager::Initialize()
{
	...
	{
		bEnableDatabaseMirroring = !!iTemp;
		fprintf(stdout, "MIRROR2DB: %s\n", (bEnableDatabaseMirroring)?"Enabled":"Disabled");
	}
#endif
...
void CClientManager::BlockChat(TPacketBlockChat* p)
{
	...
	if (pRes->uiNumRows)
	{
		...
		packet.playerId = pid;
		packet.affectElements.type = 223;
		packet.affectElements.applyOn = 0;
		packet.affectElements.applyValue = 0;
		packet.affectElements.flag = 0;
		packet.affectElements.duration = p->lDuration;
		packet.affectElements.spCost = 0;
		QUERY_ADD_AFFECT(NULL, &packet);
	}
	else
	{
...
void CClientManager::MyshopPricelistUpdate(const TItemPriceListTable* packet)
{
	if (!packet) 
	{
		sys_err("UpdateMyshopPriceList: Received null packet pointer.");
		return;
	}	

	if (packet->byCount > SHOP_PRICELIST_MAX_NUM) 
	{
		sys_err("UpdateMyshopPriceList: Item count exceeds maximum limit.");
		return;
	}

	auto* priceListCache = GetItemPriceListCache(packet->dwOwnerID);

	if (priceListCache)
	{
		TItemPriceListTable updatedTable;
		updatedTable.dwOwnerID = packet->dwOwnerID;
		updatedTable.byCount = packet->byCount;

		std::copy(packet->aPriceInfo, packet->aPriceInfo + packet->byCount, updatedTable.aPriceInfo);

		priceListCache->UpdateList(&updatedTable);
		priceListCache->OnFlush();
	}
	else
	{
		auto updatedTablePtr = std::make_unique<TItemPriceListTable>();
		updatedTablePtr->dwOwnerID = packet->dwOwnerID;
		updatedTablePtr->byCount = packet->byCount;

		std::copy(packet->aPriceInfo, packet->aPriceInfo + packet->byCount, updatedTablePtr->aPriceInfo);

		char query[QUERY_MAX_LEN];
		snprintf(query, sizeof(query), "SELECT item_vnum, price FROM myshop_pricelist WHERE owner_id=%u", updatedTablePtr->dwOwnerID);
		CDBManager::Instance().ReturnQuery(query, QID_ITEMPRICE_LOAD_FOR_UPDATE, 0, updatedTablePtr.release());
	}
}
...
void CClientManager::SendRemoveAllFriendsPacket(const char* playerName)
{
	TPacketDGRemoveAllFriends removeAllFriendsPacket;
	removeAllFriendsPacket.bHeader = HEADER_DG_REMOVE_ALL_FRIENDS;
	strncpy(removeAllFriendsPacket.szPlayerName, playerName, sizeof(removeAllFriendsPacket.szPlayerName) - 1);
	removeAllFriendsPacket.szPlayerName[sizeof(removeAllFriendsPacket.szPlayerName) - 1] = '\0';
	ForwardPacket(HEADER_DG_REMOVE_ALL_FRIENDS, &removeAllFriendsPacket, sizeof(TPacketDGRemoveAllFriends));
}