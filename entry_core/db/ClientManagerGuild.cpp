...
void CClientManager::GuildRemoveMember(CPeer* peer, TPacketGuild* p)
{
	...
	snprintf(szQuery, sizeof(szQuery), "REPLACE INTO quest%s (dwPID, szName, szState, lValue) VALUES(%u, 'guild_manager', 'member_rejoin_cooldown', %u)", GetTablePostfix(), p->dwInfo, (DWORD) GetCurrentTime());
	CDBManager::instance().AsyncQuery(szQuery);

	ForwardPacket(HEADER_DG_GUILD_REMOVE_MEMBER, p, sizeof(TPacketGuild));
}

void CClientManager::SendGuildRemoveMemberPacket(DWORD guildID, DWORD playerID)
{
    TPacketGuild guildPacket;
    guildPacket.dwGuild = guildID;
    guildPacket.dwInfo = playerID;
    
    ForwardPacket(HEADER_DG_GUILD_REMOVE_MEMBER, &guildPacket, sizeof(TPacketGuild));

    const char* playerName = GetCharacterName(playerID);
    if (playerName && playerName[0] != '\0')
    {
        TPacketGuildCommentRemove commentRemovePacket;
        commentRemovePacket.dwGuild = guildID;
        strncpy(commentRemovePacket.szName, playerName, sizeof(commentRemovePacket.szName) - 1);
        commentRemovePacket.szName[sizeof(commentRemovePacket.szName) - 1] = '\0';
        ForwardPacket(HEADER_DG_GUILD_COMMENT_REMOVE, &commentRemovePacket, sizeof(TPacketGuildCommentRemove));
	}
}

DWORD CClientManager::GetGuildIDByPlayerID(DWORD playerID)
{
	char szQuery[256];
	snprintf(szQuery, sizeof(szQuery), "SELECT guild_id FROM guild_member%s WHERE pid=%u", GetTablePostfix(), playerID);
	std::unique_ptr<SQLMsg> pGuildMsg(CDBManager::instance().DirectQuery(szQuery));

	if (pGuildMsg->Get() && pGuildMsg->Get()->uiNumRows)
	{
		MYSQL_ROW guildRow = mysql_fetch_row(pGuildMsg->Get()->pSQLResult);
		DWORD dwGuildID = 0;
...
const char* CClientManager::GetCharacterName(DWORD playerID)
{
	char szQuery[256];
	snprintf(szQuery, sizeof(szQuery), "SELECT name FROM player%s WHERE id=%u", GetTablePostfix(), playerID);
	std::unique_ptr<SQLMsg> pPlayerMsg(CDBManager::instance().DirectQuery(szQuery));

	if (pPlayerMsg->Get() && pPlayerMsg->Get()->uiNumRows)
	{
		MYSQL_ROW playerRow = mysql_fetch_row(pPlayerMsg->Get()->pSQLResult);
		return playerRow[0];
	}
...
void CClientManager::GuildDisband(CPeer* peer, TPacketGuild* p)
{
	...
	snprintf(szQuery, sizeof(szQuery), "REPLACE INTO quest%s (dwPID, szName, szState, lValue) SELECT pid, 'guild_manager', 'guild_reformation_cooldown', %u FROM guild_member%s WHERE guild_id = %u", GetTablePostfix(), (DWORD) GetCurrentTime(), GetTablePostfix(), p->dwGuild);
	CDBManager::instance().AsyncQuery(szQuery);
