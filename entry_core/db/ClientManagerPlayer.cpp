...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		if (pSet)
		{
			...
			// Affect
			snprintf(szQuery, sizeof(szQuery), "SELECT player_id, type, apply_on, apply_value, flag, duration, sp_cost FROM affect WHERE player_id=%d", pTab->id);
		...
		else
		{
			...
			CDBManager::instance().ReturnQuery(szQuery,
					...
					new ClientHandleInfo(dwHandle, pTab->id));
			snprintf(szQuery, sizeof(szQuery), 
					"SELECT player_id, type, apply_on, apply_value, flag, duration, sp_cost FROM affect WHERE player_id=%d",
					GetTablePostfix(), pTab->id);
	...
	else
	{
		...
		//--------------------------------------------------------------
		snprintf(queryStr, sizeof(queryStr),
				"SELECT player_id, type, apply_on, apply_value, flag, duration, sp_cost FROM affect WHERE player_id=%d", packet->player_id);
...
void CClientManager::RESULT_AFFECT_LOAD(CPeer* peer, MYSQL_RES* pRes, DWORD dwHandle, DWORD dwClientPID)
{
	int iNumRows = mysql_num_rows(pRes);

	if (iNumRows == 0)
	{
		sys_log(0, "AFFECT_LOAD: No affects to load for PID %u", dwClientPID);
		
		DWORD dwCount = 0;
		peer->EncodeHeader(HEADER_DG_AFFECT_LOAD, dwHandle, sizeof(dwClientPID) + sizeof(dwCount));
		peer->Encode(&dwClientPID, sizeof(dwClientPID));
		peer->Encode(&dwCount, sizeof(dwCount));
		return;
	}

	std::vector<TPacketAffectElement> elements;
	elements.resize(iNumRows);
	...
	{
		TPacketAffectElement& affectElements = elements[i];
		row = mysql_fetch_row(pRes);

		if (playerId == 0)
			str_to_number(playerId, row[0]);

		str_to_number(affectElements.type, row[1]);
		str_to_number(affectElements.applyOn, row[2]);
		str_to_number(affectElements.applyValue, row[3]);
		str_to_number(affectElements.flag, row[4]);
		str_to_number(affectElements.duration, row[5]);
		str_to_number(affectElements.spCost, row[6]);
	}

	sys_log(0, "AFFECT_LOAD: Loaded %u affects for PID %u", elements.size(), playerId);

	DWORD dwCount = elements.size();

	peer->EncodeHeader(HEADER_DG_AFFECT_LOAD, dwHandle, sizeof(playerId) + sizeof(dwCount) + sizeof(TPacketAffectElement) * dwCount);
	peer->Encode(&playerId, sizeof(playerId));
	peer->Encode(&dwCount, sizeof(dwCount));
	peer->Encode(elements.data(), sizeof(TPacketAffectElement) * dwCount);
}
...
void CClientManager::__RESULT_PLAYER_DELETE(CPeer *peer, SQLMsg* msg)
{
	...
	if (msg->Get() && msg->Get()->uiNumRows)
	{
		...
		DWORD dwGuildID = GetGuildIDByPlayerID(dwPID);
		if (dwGuildID != 0)
		{
			SendGuildRemoveMemberPacket(dwGuildID, dwPID);
		}

		SendRemoveAllFriendsPacket(szName);
		...
		snprintf(queryStr, sizeof(queryStr), "DELETE FROM affect WHERE player_id=%d", pi->player_id);
		CDBManager::instance().AsyncQuery(queryStr);
...
void CClientManager::QUERY_ADD_AFFECT(CPeer * peer, TPacketGDAddAffect * p)
{
	...
	snprintf(queryStr, sizeof(queryStr),
			"REPLACE INTO affect (player_id, type, apply_on, apply_value, flag, duration, sp_cost) "
			"VALUES(%u, %u, %u, %ld, %u, %ld, %ld)",
			p->playerId,
			p->affectElements.type,
			p->affectElements.applyOn,
			p->affectElements.applyValue,
			p->affectElements.flag,
			p->affectElements.duration,
			p->affectElements.spCost);
...
{
	char queryStr[QUERY_MAX_LEN];
	snprintf(queryStr, sizeof(queryStr), "DELETE FROM affect WHERE player_id=%u AND type=%u AND apply_on=%u", p->playerId, p->type, p->applyOn);
	CDBManager::instance().AsyncQuery(queryStr);
}
